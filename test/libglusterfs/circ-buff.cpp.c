...
{
        circular_buffer_t *ptr = ((void *)0);
        int ret = -1;

        do { if (!(buffer->used_len <= buffer->size_buffer)) { do { _gf_msg ("", "./circ-buff.c", __FUNCTION__, 32, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "buffer->used_len <= buffer->size_buffer"); } while (0); } } while (0);

        if (buffer->use_once == _gf_true &&
            buffer->used_len == buffer->size_buffer) {
                do { _gf_msg ("circ-buff",
 "./circ-buff.c"
# 36 "./circ-buff.c"
                , __FUNCTION__,
 37
# 36 "./circ-buff.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 23), "buffer %p is use once buffer", buffer); } while (0)
                                                                ;
                return -1;
        } else {
                if (buffer->used_len == buffer->size_buffer) {
                        if (buffer->cb[buffer->w_index]) {
                                ...
                                buffer->cb[buffer->w_index] = ((void *)0);
                                ptr = ((void *)0);
                        }
                }

                buffer->cb[buffer->w_index] =
                        __gf_calloc (1, sizeof (circular_buffer_t), gf_common_mt_circular_buffer_t, "gf_common_mt_circular_buffer_t")
                                                                  ;
                if (!buffer->cb[buffer->w_index])
                        return -1;

                buffer->cb[buffer->w_index]->data = item;
                ret = gettimeofday (&buffer->cb[buffer->w_index]->tv, ((void *)0));
                if (ret == -1)
                        do { _gf_msg ("circ-buff",
...
# 63 "./circ-buff.c"
 ...
 65
# 63 "./circ-buff.c"
                        , GF_LOG_WARNING, 0, 1, (100000 + 1000 + 175), "getting time of the day failed"); } while (0)
...
# 95 "./circ-buff.c"
 ...
 97
# 95 "./circ-buff.c"
                , GF_LOG_DEBUG, 0, 0, 0, "w_index: %d, size: %" "zu"" used_buffer: %d", buffer->w_index, buffer->size_buffer, buffer->used_len); } while (0)
...
{
        int index = 0;
        circular_buffer_t *entry = ((void *)0);
        int entries = 0;
        int ul = 0;
        int w_ind = 0;
        int size_buff = 0;
...
# 128 "./circ-buff.c"
 ...
 133
# 128 "./circ-buff.c"
                                        , GF_LOG_WARNING, 0, 1, (100000 + 1000 + 148), "Null entry in " "circular buffer at " "index %d.", index); } while (0)
...
cb_buffer_new (size_t buffer_size, gf_boolean_t use_once,
               void (*destroy_buffer_data) (void *data))
{
        buffer_t *buffer = ((void *)0);

        buffer = __gf_calloc (1, sizeof (*buffer), gf_common_mt_buffer_t, "gf_common_mt_buffer_t");
        if (!buffer) {
                goto out;
        }

        buffer->cb = __gf_calloc (buffer_size, sizeof (circular_buffer_t *), gf_common_mt_circular_buffer_t, "gf_common_mt_circular_buffer_t")
        ...
        pthread_mutex_init (&buffer->lock, ((void *)0));
