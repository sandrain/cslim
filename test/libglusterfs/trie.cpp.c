...
trie_new ()
{
        ...
        trie = __gf_calloc (1, sizeof (*trie), gf_common_mt_trie_trie, "gf_common_mt_trie_trie");
        if (!trie)
                return ((void *)0);
...
{
        trienode_t *subnode = ((void *)0);

        subnode = node->subnodes[id];
        if (!subnode) {
                subnode = __gf_calloc (1, sizeof (*subnode), gf_common_mt_trie_node, "gf_common_mt_trie_node")
                                                            ;
                if (!subnode)
                        return ((void *)0);
...
trienode_get_word (trienode_t *node, char **bufp)
{
        ...
        buf = __gf_calloc (1, node->depth + 1, gf_common_mt_trie_buf, "gf_common_mt_trie_buf");
        if (!buf)
                return -1;
        *bufp = buf;
...
calc_dist (trienode_t *node, void *data)
{
        ...
        node->data = __gf_calloc (node->trie->len, sizeof (int), gf_common_mt_trie_data, "gf_common_mt_trie_data")
                                                       ;
        if (!node->data)
                return -1;
        row = node->data;
        ...
        for (i = 0; i < node->trie->len; i++) {
                ...
                if (word[i] == node->id)
                        ...
                        row[i] = (((distul + 1))<((((distu + 1))<((distl + 1))?((distu + 1)):((distl + 1))))?((distul + 1)):((((distu + 1))<((distl + 1))?((distu + 1)):((distl + 1)))))
...
{
        struct trienodevec_w *nodevec_w = ((void *)0);
        struct trienodevec *nodevec = ((void *)0);
        int dist = 0;
        int i = 0;
...
trie_reset_search (trie_t *trie)
{
        ...
        trie_walk (trie, trienode_reset, ((void *)0), 0);
}
