...
gf_add_prefix(const char *ns, const char *key, char **newkey)
{
        ...
        if (strncmp(key, "user.", (sizeof ("user.") - 1)) &&
            strncmp(key, "trusted.", (sizeof ("trusted.") - 1)) &&
            strncmp(key, "security.", (sizeof ("trusted.") - 1)) &&
            strncmp(key, "system.", (sizeof ("system.") - 1))) {
                int ns_length = strlen(ns);
                *newkey = __gf_malloc (ns_length + strlen(key) + 10, gf_common_mt_char, "gf_common_mt_char")
                                                      ;
                strcpy(*newkey, ns);
                strcat(*newkey, key);
        } else {
