...
void _gf_proc_dump_build_key (char *key, const char *prefix, char *fmt,...)
{
        ...
        __builtin_va_end(ap);
        snprintf(key, 4096, "%s.%s", prefix, buf);
}
# 68 "./statedump.h"
...
gf_iobuf_get_arena_index (size_t page_size)
{
        ...
        for (i = 0; i < (sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config))); i++) {
                if (page_size <= gf_iobuf_init_config[i].pagesize)
                        break;
        }

        if (i >= (sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config))))
                i = -1;
...
{
        int iobuf_cnt = 0;
        struct iobuf *iobuf = ((void *)0);
        int offset = 0;
        int i = 0;

        do { if (!iobuf_arena) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 80, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_arena"); } while (0); goto out; } } while (0);

        iobuf_cnt = iobuf_arena->page_count;

        iobuf_arena->iobufs = __gf_calloc (sizeof (*iobuf), iobuf_cnt, gf_common_mt_iobuf, "gf_common_mt_iobuf")
                                                            ;
        ...
        for (i = 0; i < iobuf_cnt; i++) {
                do { (&iobuf->list)->next = (&iobuf->list)->prev = &iobuf->list; } while (0);
                pthread_mutex_init (&iobuf->lock, 0);
...
__iobuf_arena_destroy_iobufs (struct iobuf_arena *iobuf_arena)
{
        ...
        do { if (!iobuf_arena) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 117, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_arena"); } while (0); goto out; } } while (0);
...
# 122 "./iobuf.c"
 ...
 123
# 122 "./iobuf.c"
                , GF_LOG_ERROR, 0, 1, (100000 + 1000 + 143), "iobufs not found"); } while (0)
                                                                              ;
                return;
        }
        ...
        iobuf = iobuf_arena->iobufs;
        for (i = 0; i < iobuf_cnt; i++) {
                do { if (!(iobuf->ref == 0)) { do { _gf_msg ("", "./iobuf.c", __FUNCTION__, 129, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "iobuf->ref == 0"); } while (0); } } while (0);

                pthread_mutex_destroy (&iobuf->lock);
                list_del_init (&iobuf->list);
                iobuf++;
        }

        __gf_free (iobuf_arena->iobufs);
...
__iobuf_arena_destroy (struct iobuf_pool *iobuf_pool,
                       struct iobuf_arena *iobuf_arena)
{
        do { if (!iobuf_arena) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 147, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_arena"); } while (0); goto out; } } while (0);
...
__iobuf_arena_alloc (struct iobuf_pool *iobuf_pool, size_t page_size,
                     int32_t num_iobufs)
{
        struct iobuf_arena *iobuf_arena = ((void *)0);
        size_t rounded_size = 0;

        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 172, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);

        iobuf_arena = __gf_calloc (sizeof (*iobuf_arena), 1, gf_common_mt_iobuf_arena, "gf_common_mt_iobuf_arena")
                                                          ;
        if (!iobuf_arena)
                goto err;

        do { (&iobuf_arena->list)->next = (&iobuf_arena->list)->prev = &iobuf_arena->list; } while (0);
        do { (&iobuf_arena->all_list)->next = (&iobuf_arena->all_list)->prev = &iobuf_arena->all_list; } while (0);
        do { (&iobuf_arena->active.list)->next = (&iobuf_arena->active.list)->prev = &iobuf_arena->active.list; } while (0);
        do { (&iobuf_arena->passive.list)->next = (&iobuf_arena->passive.list)->prev = &iobuf_arena->passive.list; } while (0);
        ...
        iobuf_arena->mem_base = mmap (((void *)0), iobuf_arena->arena_size,
                                      0x1|0x2,
                                      0x02|0x20, -1, 0);
...
# 196 "./iobuf.c"
 ...
 197
# 196 "./iobuf.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 34), "mapping failed"); } while (0)
                                         ;
                goto err;
        }
...
# 210 "./iobuf.c"
 ...
 211
# 210 "./iobuf.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 35), "init failed"); } while (0)
                                      ;
                goto err;
        }
...
{
        struct iobuf_arena *iobuf_arena = ((void *)0);
        struct iobuf_arena *tmp = ((void *)0);
        int index = 0;

        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 234, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
...
# 238 "./iobuf.c"
 ...
 240
# 238 "./iobuf.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 36), "page_size (%zu) of iobufs in arena being added is " "greater than max available", page_size); } while (0)

                                                                ;
                return ((void *)0);
        }

        for (tmp = ((typeof(*tmp) *)((char *)((&iobuf_pool->purge[index])->next)-(unsigned long)(&((typeof(*tmp) *)0)->list))); &tmp->list != (&iobuf_pool->purge[index]); tmp = ((typeof(*tmp) *)((char *)(tmp->list.next)-(unsigned long)(&((typeof(*tmp) *)0)->list)))) {
...
# 276 "./iobuf.c"
 ...
 277
# 276 "./iobuf.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 37), "arena not found"); } while (0)
                                          ;
                return ((void *)0);
        }
                      ...
                      int32_t num_pages)
{
        ...
        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 293, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
...
iobuf_pool_destroy (struct iobuf_pool *iobuf_pool)
{
        ...
        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 315, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);

        pthread_mutex_lock (&iobuf_pool->mutex);
        {
                for (i = 0; i < (sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config))); i++) {
                        for (iobuf_arena = ((typeof(*iobuf_arena) *)((char *)((&iobuf_pool->arenas[i])->next)-(unsigned long)(&((typeof(*iobuf_arena) *)0)->list))), tmp = ((typeof(*iobuf_arena) *)((char *)(iobuf_arena->list.next)-(unsigned long)(&((typeof(*iobuf_arena) *)0)->list))); &iobuf_arena->list != (&iobuf_pool->arenas[i]); iobuf_arena = tmp, tmp = ((typeof(*tmp) *)((char *)(tmp->list.next)-(unsigned long)(&((typeof(*tmp) *)0)->list))))
                                                                      {
                        ...
                        for (iobuf_arena = ((typeof(*iobuf_arena) *)((char *)((&iobuf_pool->purge[i])->next)-(unsigned long)(&((typeof(*iobuf_arena) *)0)->list))), tmp = ((typeof(*iobuf_arena) *)((char *)(iobuf_arena->list.next)-(unsigned long)(&((typeof(*iobuf_arena) *)0)->list))); &iobuf_arena->list != (&iobuf_pool->purge[i]); iobuf_arena = tmp, tmp = ((typeof(*tmp) *)((char *)(tmp->list.next)-(unsigned long)(&((typeof(*tmp) *)0)->list))))
                                                                     {
                                list_del_init (&iobuf_arena->list);
                                iobuf_pool->arena_cnt--;
                                __iobuf_arena_destroy (iobuf_pool, iobuf_arena);
                        ...
                        for (iobuf_arena = ((typeof(*iobuf_arena) *)((char *)((&iobuf_pool->filled[i])->next)-(unsigned long)(&((typeof(*iobuf_arena) *)0)->list))), tmp = ((typeof(*iobuf_arena) *)((char *)(iobuf_arena->list.next)-(unsigned long)(&((typeof(*iobuf_arena) *)0)->list))); &iobuf_arena->list != (&iobuf_pool->filled[i]); iobuf_arena = tmp, tmp = ((typeof(*tmp) *)((char *)(tmp->list.next)-(unsigned long)(&((typeof(*tmp) *)0)->list))))
                                                                      {
                                list_del_init (&iobuf_arena->list);
                                iobuf_pool->arena_cnt--;
                                __iobuf_arena_destroy (iobuf_pool, iobuf_arena);
...
iobuf_create_stdalloc_arena (struct iobuf_pool *iobuf_pool)
{
        ...
        list_add_tail (&iobuf_arena->list,
                       &iobuf_pool->arenas[(sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config)))]);
...
iobuf_pool_new (void)
{
        struct iobuf_pool *iobuf_pool = ((void *)0);
        int i = 0;
        size_t page_size = 0;
        size_t arena_size = 0;
        int32_t num_pages = 0;

        iobuf_pool = __gf_calloc (sizeof (*iobuf_pool), 1, gf_common_mt_iobuf_pool, "gf_common_mt_iobuf_pool")
                                                        ;
        if (!iobuf_pool)
                goto out;
        do { (&iobuf_pool->all_arenas)->next = (&iobuf_pool->all_arenas)->prev = &iobuf_pool->all_arenas; } while (0);
        pthread_mutex_init (&iobuf_pool->mutex, ((void *)0));
        for (i = 0; i <= (sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config))); i++) {
                do { (&iobuf_pool->arenas[i])->next = (&iobuf_pool->arenas[i])->prev = &iobuf_pool->arenas[i]; } while (0);
                do { (&iobuf_pool->filled[i])->next = (&iobuf_pool->filled[i])->prev = &iobuf_pool->filled[i]; } while (0);
                do { (&iobuf_pool->purge[i])->next = (&iobuf_pool->purge[i])->prev = &iobuf_pool->purge[i]; } while (0);
        }

        iobuf_pool->default_page_size = 128 * 1024ULL;

        iobuf_pool->rdma_registration = ((void *)0);
        iobuf_pool->rdma_deregistration = ((void *)0);

        for (i = 0; i < 8; i++) {

                iobuf_pool->device[i] = ((void *)0);
                iobuf_pool->mr_list[i] = ((void *)0);
...
{
        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 444, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
...
iobuf_pool_prune (struct iobuf_pool *iobuf_pool)
{
        ...
        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 473, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
...
{
        struct iobuf_arena *iobuf_arena = ((void *)0);
        struct iobuf_arena *trav = ((void *)0);
        int index = 0;

        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 502, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
...
# 506 "./iobuf.c"
        ...
        for (trav = ((typeof(*trav) *)((char *)((&iobuf_pool->arenas[index])->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&iobuf_pool->arenas[index]); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list)))) {
                if (trav->passive_cnt) {
                        iobuf_arena = trav;
                        break;
                }
...
__iobuf_get (struct iobuf_arena *iobuf_arena, size_t page_size)
{
        ...
        do { if (!iobuf_arena) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 555, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_arena"); } while (0); goto out; } } while (0);

        iobuf_pool = iobuf_arena->iobuf_pool;

        for (iobuf = ((typeof(*iobuf) *)((char *)((&iobuf_arena->passive.list)->next)-(unsigned long)(&((typeof(*iobuf) *)0)->list))); &iobuf->list != (&iobuf_arena->passive.list); iobuf = ((typeof(*iobuf) *)((char *)(iobuf->list.next)-(unsigned long)(&((typeof(*iobuf) *)0)->list))))
                break;
...
iobuf_get_from_stdalloc (struct iobuf_pool *iobuf_pool, size_t page_size)
{
        ...
        for (trav = ((typeof(*trav) *)((char *)((&iobuf_pool->arenas[(sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config)))])->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&iobuf_pool->arenas[(sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config)))]); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list))))
                                   {
                iobuf_arena = trav;
                break;
        }

        iobuf = __gf_calloc (1, sizeof (*iobuf), gf_common_mt_iobuf, "gf_common_mt_iobuf");
        if (!iobuf)
                goto out;


        iobuf->free_ptr = __gf_calloc (1, ((page_size + 512) - 1), gf_common_mt_char, "gf_common_mt_char")
                                                       ;
        if (!iobuf->free_ptr)
                goto out;

        iobuf->ptr = ((void *)((unsigned long)(iobuf->free_ptr + 512 - 1) & (unsigned long)(~(512 - 1))));
        iobuf->iobuf_arena = iobuf_arena;
        pthread_mutex_init (&iobuf->lock, 0);
...
# 653 "./iobuf.c"
 ...
 656
# 653 "./iobuf.c"
                , GF_LOG_DEBUG, 0, 0, 0, "request for iobuf of size %zu " "is serviced using standard calloc() (%p) as it " "exceeds the maximum available buffer size", page_size, iobuf); } while (0)
...
iobuf_get (struct iobuf_pool *iobuf_pool)
{
        ...
        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 687, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
...
# 703 "./iobuf.c"
 ...
 704
# 703 "./iobuf.c"
                        , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 38), "iobuf not found"); } while (0)
                                                                          ;
                        goto unlock;
                }
...
__iobuf_put (struct iobuf *iobuf, struct iobuf_arena *iobuf_arena)
{
        ...
        do { if (!iobuf_arena) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 723, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_arena"); } while (0); goto out; } } while (0);
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 724, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
# 730 "./iobuf.c"
 ...
 731
# 730 "./iobuf.c"
                , GF_LOG_DEBUG, 0, 0, 0, "freeing the iobuf (%p) " "allocated with standard calloc()", iobuf); } while (0)
                                                                  ;
...
iobuf_put (struct iobuf *iobuf)
{
        ...
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 767, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
# 778 "./iobuf.c"
 ...
 779
# 778 "./iobuf.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 39), "iobuf pool not found"); } while (0)
                                                                      ;
                return;
        }
...
iobuf_unref (struct iobuf *iobuf)
{
        ...
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 799, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
{
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 819, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
iobref_new ()
{
        struct iobref *iobref = ((void *)0);

        iobref = __gf_calloc (sizeof (*iobref), 1, gf_common_mt_iobref, "gf_common_mt_iobref")
                                                ;
        if (!iobref)
                return ((void *)0);

 iobref->iobrefs = __gf_calloc (sizeof (*iobref->iobrefs), 16, gf_common_mt_iobrefs, "gf_common_mt_iobrefs")
                                  ;
 if (!iobref->iobrefs) {
  __gf_free (iobref);
  return ((void *)0);
 ...
 iobref->used = 0;
        ...
        pthread_mutex_init (&iobref->lock, 0);
...
{
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 863, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);

        pthread_mutex_lock (&iobref->lock);
        {
                iobref->ref++;
        }
        pthread_mutex_unlock (&iobref->lock);
...
iobref_destroy (struct iobref *iobref)
{
        ...
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 882, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);

        for (i = 0; i < iobref->alloced; i++) {
                iobuf = iobref->iobrefs[i];

                iobref->iobrefs[i] = ((void *)0);
                if (iobuf)
                        iobuf_unref (iobuf);
        }

 __gf_free (iobref->iobrefs);
        __gf_free (iobref);
...
iobref_unref (struct iobref *iobref)
{
        ...
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 905, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);
...
iobref_clear (struct iobref *iobref)
{
        ...
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 926, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);

        for (; i < iobref->alloced; i++) {
                if (iobref->iobrefs[i] != ((void *)0)) {
...
__iobref_grow (struct iobref *iobref)
{
 ...
 newptr = __gf_realloc (iobref->iobrefs, iobref->alloced * 2 * (sizeof (*iobref->iobrefs)))
                                                          ;
 if (newptr) {
...
__iobref_add (struct iobref *iobref, struct iobuf *iobuf)
{
        ...
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 968, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 969, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
iobref_add (struct iobref *iobref, struct iobuf *iobuf)
{
        ...
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 999, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1000, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
iobref_merge (struct iobref *to, struct iobref *from)
{
        ...
        do { if (!to) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1020, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "to"); } while (0); goto out; } } while (0);
        do { if (!from) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1021, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "from"); } while (0); goto out; } } while (0);
...
iobuf_size (struct iobuf *iobuf)
{
        ...
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1049, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
...
# 1058 "./iobuf.c"
 ...
 1059
# 1058 "./iobuf.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 39), "pool not found"); } while (0)
                                         ;
                goto out;
        }
...
iobref_size (struct iobref *iobref)
{
        ...
        do { if (!iobref) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1075, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobref"); } while (0); goto out; } } while (0);
...
iobuf_info_dump (struct iobuf *iobuf, const char *key_prefix)
{
        ...
        do { if (!iobuf) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1097, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf"); } while (0); goto out; } } while (0);
        ...
        { _gf_proc_dump_build_key(key, key_prefix,"ref"); };
        gf_proc_dump_write(key, "%d", my_iobuf.ref);
        { _gf_proc_dump_build_key(key, key_prefix,"ptr"); };
...
iobuf_arena_info_dump (struct iobuf_arena *iobuf_arena, const char *key_prefix)
{
        ...
        do { if (!iobuf_arena) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1124, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_arena"); } while (0); goto out; } } while (0);

        { _gf_proc_dump_build_key(key, key_prefix,"mem_base"); };
        gf_proc_dump_write(key, "%p", iobuf_arena->mem_base);
        { _gf_proc_dump_build_key(key, key_prefix, "active_cnt"); };
        gf_proc_dump_write(key, "%d", iobuf_arena->active_cnt);
        { _gf_proc_dump_build_key(key, key_prefix, "passive_cnt"); };
        gf_proc_dump_write(key, "%d", iobuf_arena->passive_cnt);
        { _gf_proc_dump_build_key(key, key_prefix, "alloc_cnt"); };
        gf_proc_dump_write(key, "%""l" "u", iobuf_arena->alloc_cnt);
        { _gf_proc_dump_build_key(key, key_prefix, "max_active"); };
        gf_proc_dump_write(key, "%""l" "u", iobuf_arena->max_active);
        { _gf_proc_dump_build_key(key, key_prefix, "page_size"); };
        gf_proc_dump_write(key, "%""l" "u", iobuf_arena->page_size);
        for (trav = ((typeof(*trav) *)((char *)((&iobuf_arena->active.list)->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&iobuf_arena->active.list); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list)))) {
                { _gf_proc_dump_build_key(key, key_prefix,"active_iobuf.%d", i++); };
...
iobuf_stats_dump (struct iobuf_pool *iobuf_pool)
{
        ...
        do { if (!iobuf_pool) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1157, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iobuf_pool"); } while (0); goto out; } } while (0);
        ...
        gf_proc_dump_write("iobuf_pool.arena_cnt", "%d",
                           iobuf_pool->arena_cnt);
        gf_proc_dump_write("iobuf_pool.request_misses", "%""l" "d",
                           iobuf_pool->request_misses);

        for (j = 0; j < (sizeof (gf_iobuf_init_config) / (sizeof (struct iobuf_init_config))); j++) {
                for (trav = ((typeof(*trav) *)((char *)((&iobuf_pool->arenas[j])->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&iobuf_pool->arenas[j]); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list)))) {
                ...
                for (trav = ((typeof(*trav) *)((char *)((&iobuf_pool->purge[j])->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&iobuf_pool->purge[j]); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list)))) {
                        snprintf(msg, sizeof(msg),
                                 "purge.%d", i);
                        gf_proc_dump_add_section(msg);
                        iobuf_arena_info_dump(trav,msg);
                ...
                for (trav = ((typeof(*trav) *)((char *)((&iobuf_pool->filled[j])->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&iobuf_pool->filled[j]); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list)))) {
                        snprintf(msg, sizeof(msg),
                                 "filled.%d", i);
                        gf_proc_dump_add_section(msg);
                        iobuf_arena_info_dump(trav,msg);
...
{
        do { if (!iob) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1212, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iob"); } while (0); goto out; } } while (0);
        do { if (!iov) { (*__errno_location ()) = 22; do { _gf_msg ("iobuf", "./iobuf.c", __FUNCTION__, 1213, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "iov"); } while (0); goto out; } } while (0);

        iov->iov_base = ((iob)->ptr);
        iov->iov_len = (iob->iobuf_arena->page_size);
