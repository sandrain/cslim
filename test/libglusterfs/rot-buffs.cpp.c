...
{
        pthread_mutex_lock (&rlist->c_lock);
        {
                rlist->awaiting = _gf_true;
        }
        pthread_mutex_unlock (&rlist->c_lock);
}
...
rbuf_alloc_rvec ()
{
        return __gf_calloc (1, ((sizeof (rbuf_iovec_t)) + (1 * 1024 * 1024)), gf_common_mt_rvec_t, "gf_common_mt_rvec_t");
}
...
{
        rbuf_iovec_t *rvec = ((void *)0);

        rvec = (rbuf_iovec_t *) rbuf_alloc_rvec ();
        if (!rvec)
                return -1;
        do { (&rvec->list)->next = (&rvec->list)->prev = &rvec->list; } while (0);
        rvec->iov.iov_base = ((char *)rvec) + (sizeof (rbuf_iovec_t));
        rvec->iov.iov_len = 0;
...
rlist_purge_all_rvec (rbuf_list_t *rlist)
{
        ...
        while (!list_empty (&rlist->veclist)) {
                rvec = ((rbuf_iovec_t *)((char *)((&rlist->veclist)->next)-(unsigned long)(&((rbuf_iovec_t *)0)->list)));
...
{
        rbuf_list_t *rlist = ((void *)0);

        while (!list_empty (&rbuf->freelist)) {
                rlist = ((rbuf_list_t *)((char *)((&rbuf->freelist)->next)-(unsigned long)(&((rbuf_list_t *)0)->list)));
                list_del (&rlist->list);

                rlist_purge_all_rvec (rlist);

                pthread_mutex_destroy (&rlist->c_lock);
...
rbuf_init (int bufcount)
{
        ...
        rbuf = __gf_calloc (1, sizeof (rbuf_t), gf_common_mt_rbuf_t, "gf_common_mt_rbuf_t");
        if (!rbuf)
                goto error_return;

        pthread_mutex_init (&rbuf->lock, 0);
        do { (&rbuf->freelist)->next = (&rbuf->freelist)->prev = &rbuf->freelist; } while (0);


        for (j = 0; j < bufcount; j++) {
                rlist = __gf_calloc (1, sizeof (rbuf_list_t), gf_common_mt_rlist_t, "gf_common_mt_rlist_t")
                                                                              ;
                if (!rlist) {
                        ret = -1;
                        break;
                ...
                do { (&rlist->list)->next = (&rlist->list)->prev = &rlist->list; } while (0);
                do { (&rlist->veclist)->next = (&rlist->veclist)->prev = &rlist->veclist; } while (0);
                ...
                pthread_mutex_init (&rlist->c_lock, 0);
        ...
        rbuf->current = ((rbuf_list_t *)((char *)((&rbuf->freelist)->next)-(unsigned long)(&((rbuf_list_t *)0)->list)));
        return rbuf;
...
rbuf_alloc_write_area (rbuf_list_t *rlist, size_t bytes)
{
        ...
        iov = &rlist->rvec->iov;
        if (iov->iov_len + bytes <= (1 * 1024 * 1024))
                return rbuf_adjust_write_area (iov, bytes);


        if (list_is_last (&rlist->rvec->list, &rlist->veclist)) {

                do { if (!(rlist->used == rlist->total)) { do { _gf_msg ("", "./rot-buffs.c", __FUNCTION__, 284, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "rlist->used == rlist->total"); } while (0); } } while (0);
                ret = rlist_add_new_vec (rlist);
                if (ret)
                        goto error_return;
        } else {

                rlist->rvec = ((typeof(*(rlist->rvec)) *)((char *)((rlist->rvec)->list.next)-(unsigned long)(&((typeof(*(rlist->rvec)) *)0)->list)));
                rlist->used++;
...
rbuf_reserve_write_area (rbuf_t *rbuf, size_t bytes, void **opaque)
{
        ...
        if (!rbuf || (bytes <= 0) || (bytes > (1 * 1024 * 1024)) || !opaque)
                return ((void *)0);
                 ...
                 void **opaque, sequence_fn *seqfn, void *mydata)
{
        ...
        pthread_mutex_lock (&rbuf->lock);
        {
                ...
                if (seqfn)
                        seqfn (rlist, mydata);
                rbuf->current =
                        ((rbuf_list_t *)((char *)((&rbuf->freelist)->next)-(unsigned long)(&((rbuf_list_t *)0)->list)));
        }
 unblock:
        pthread_mutex_unlock (&rbuf->lock);
...
rlist_shrink_vector (rbuf_list_t *rlist)
{
        ...
        if (rvec_in_watermark_range (rlist))
                return;
# 444 "./rot-buffs.c"
        shrink = rlist->total - (rlist->total * pow (2.7182818284590452354, -0.2));
                          ...
                          void (*fn)(rbuf_list_t *, void *), void *arg)
{
        ...
        rlist->rvec = ((rbuf_iovec_t *)((char *)((&rlist->veclist)->next)-(unsigned long)(&((rbuf_iovec_t *)0)->list)));
        rbuf_reset_rvec (rlist->rvec);
