...
static char global_uuid_buf[50];
static pthread_key_t lkowner_buf_key;
static char global_lkowner_buf[((1024 * 2) + (1024 / 8))];
static int gf_global_mem_acct_enable = 1;
static pthread_once_t globals_inited = 0;
...
# 113 "./globals.c"
 ...
 115
# 113 "./globals.c"
                , GF_LOG_WARNING, ret, 0, (100000 + 1000 + 65), "failed to create " "the pthread key"); } while (0)
        ...
        do { (&global_xlator.volume_options)->next = (&global_xlator.volume_options)->prev = &global_xlator.volume_options; } while (0);
...
{
        xlator_t **this_location = ((void *)0);
        int ret = 0;

        this_location = pthread_getspecific (this_xlator_key);

        if (!this_location) {
                this_location = __gf_default_calloc(1,sizeof (*this_location));
                if (!this_location)
                        goto out;

                ret = pthread_setspecific (this_xlator_key, this_location);
                if (ret != 0) {
                        do { if (this_location != ((void *)0)) { free ((void *)this_location); this_location = (void *)0xeeeeeeee; } } while (0);
                        this_location = ((void *)0);
                        goto out;
                }
        }
out:
...
synctask_init ()
{
        ...
        ret = pthread_key_create (&synctask_key, ((void *)0));
...
glusterfs_lkowner_buf_get ()
{
        ...
        if(!buf) {
                buf = __gf_default_malloc(((1024 * 2) + (1024 / 8)));
...
# 340 "./globals.c"
 ...
 341
# 340 "./globals.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 66), "ERROR: glusterfs-translator init failed"); } while (0)
                                                                  ;
                goto out;
        }
...
# 347 "./globals.c"
 ...
 348
# 347 "./globals.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 67), "ERROR: glusterfs uuid buffer init failed"); } while (0)
                                                                   ;
                goto out;
        }
...
# 354 "./globals.c"
 ...
 355
# 354 "./globals.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 68), "ERROR: glusterfs lkowner buffer init failed"); } while (0)
                                                                      ;
                goto out;
        }
...
# 361 "./globals.c"
 ...
 362
# 361 "./globals.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 69), "ERROR: glusterfs synctask init failed"); } while (0)
                                                                ;
                goto out;
        }
...
# 368 "./globals.c"
 ...
 369
# 368 "./globals.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 70), "ERROR: glusterfs syncopctx init failed"); } while (0)
                                                                 ;
                goto out;
        }
out:
...
# 375 "./globals.c"
 ...
 376
# 375 "./globals.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 71), "Exiting as global initialization failed"); } while (0)
                                                                  ;
                exit (ret);
        }
}
...
# 391 "./globals.c"
 ...
 392
# 391 "./globals.c"
                , GF_LOG_CRITICAL, ret, 0, (100000 + 1000 + 72), "pthread_once failed"); } while (0)
                                              ;
