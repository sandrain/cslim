...
static strfd_t *gf_dump_strfd = ((void *)0);
...
gf_proc_dump_open (char *tmpname)
{
        ...
        mode_t mask = umask(((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3));
...
gf_proc_dump_add_section_fd (char *key, va_list ap)
{
        ...
        do { if (!(key)) { do { _gf_msg ("", "./statedump.c", __FUNCTION__, 127, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "key"); } while (0); } } while (0);

        memset (buf, 0, sizeof(buf));
        snprintf (buf, 4096, "\n[");
        vsnprintf (buf + strlen(buf),
                   4096 - strlen (buf), key, ap);
...
gf_proc_dump_write_fd (char *key, char *value, va_list ap)
{
        ...
        do { if (!(key)) { do { _gf_msg ("", "./statedump.c", __FUNCTION__, 176, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "key"); } while (0); } } while (0);
        ...
        offset += 1;
        vsnprintf (buf + offset, 4096 - offset, value, ap);
...
gf_proc_dump_mempool_info (glusterfs_ctx_t *ctx)
{
        ...
        for (pool = ((typeof(*pool) *)((char *)((&ctx->mempool_list)->next)-(unsigned long)(&((typeof(*pool) *)0)->global_list))); &pool->global_list != (&ctx->mempool_list); pool = ((typeof(*pool) *)((char *)(pool->global_list.next)-(unsigned long)(&((typeof(*pool) *)0)->global_list)))) {
                gf_proc_dump_write ("-----", "-----");
                ...
                gf_proc_dump_write ("alloc-count", "%""l" "u", pool->alloc_count);
                gf_proc_dump_write ("max-alloc", "%d", pool->max_alloc);

                gf_proc_dump_write ("pool-misses", "%""l" "u", pool->pool_misses);
...
gf_proc_dump_xlator_info (xlator_t *top)
{
        ...
        while (trav) {
                ...
                if ((dump_options.xl_options.dump_inode == _gf_true) &&
                    (trav->itable)) {
                ...
                if (trav->dumpops->priv &&
                    (dump_options.xl_options.dump_priv == _gf_true))
                ...
                if (trav->dumpops->fd &&
                    (dump_options.xl_options.dump_fd == _gf_true))
                        trav->dumpops->fd (trav);

                if (trav->dumpops->history &&
                    (dump_options.xl_options.dump_history == _gf_true))
...
gf_proc_dump_enable_all_options ()
{
        ...
        dump_options.dump_mem = _gf_true;
        dump_options.dump_iobuf = _gf_true;
        dump_options.dump_callpool = _gf_true;
        dump_options.xl_options.dump_priv = _gf_true;
        dump_options.xl_options.dump_inode = _gf_true;
        dump_options.xl_options.dump_fd = _gf_true;
        dump_options.xl_options.dump_inodectx = _gf_true
                                          ;
...
is_gf_proc_dump_all_disabled ()
{
        ...
        do { if (dump_options.dump_mem == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);;
        do { if (dump_options.dump_iobuf == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);;
        do { if (dump_options.dump_callpool == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                       ;
        do { if (dump_options.xl_options.dump_priv == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                                     ;
        do { if (dump_options.xl_options.dump_inode == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                                     ;
        do { if (dump_options.xl_options.dump_fd == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                                     ;
        do { if (dump_options.xl_options.dump_inodectx == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                                     ;
        do { if (dump_options.xl_options.dump_fdctx == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                                     ;
        do { if (dump_options.xl_options.dump_history == _gf_true) { all_disabled = _gf_false; goto out; } } while (0);
                                                     ;
...
gf_proc_dump_disable_all_options ()
{
        ...
        dump_options.dump_mem = _gf_false;
        dump_options.dump_iobuf = _gf_false;
        dump_options.dump_callpool = _gf_false;
        dump_options.xl_options.dump_priv = _gf_false;
        dump_options.xl_options.dump_inode = _gf_false
                                           ;
        dump_options.xl_options.dump_fd = _gf_false;
        dump_options.xl_options.dump_inodectx = _gf_false
                                           ;
        dump_options.xl_options.dump_fdctx = _gf_false;
        dump_options.xl_options.dump_history = _gf_false
                                           ;
        return 0;
}
...
gf_proc_dump_info (int signum, glusterfs_ctx_t *ctx)
{
        int i = 0;
        int ret = -1;
        glusterfs_graph_t *trav = ((void *)0);
        char brick_name[4096] = {0,};
        char timestr[256] = {0,};
        char sign_string[512] = {0,};
        char tmp_dump_name[4096] = {0,};
        char path[4096] = {0,};
        struct timeval tv = {0,};
        ...
        if (ctx->cmd_args.brick_name) {
                do { int i = 0; for (i = 1; i < strlen (ctx->cmd_args.brick_name); i++) { brick_name[i-1] = ctx->cmd_args.brick_name[i]; if (brick_name[i-1] == '/') brick_name[i-1] = '-'; } } while (0);
        } else
        ...
        snprintf (path, sizeof (path), "%s/%s.%d.dump.%""l" "u",
                  ((dump_options.dump_path != ((void *)0))?dump_options.dump_path:
                   ((ctx->statedump_path != ((void *)0))?ctx->statedump_path:
                    DATADIR "/run/gluster")), brick_name, getpid(),
                  (uint64_t) time (((void *)0)));

        snprintf (tmp_dump_name, 4096, "%s/dumpXXXXXX",
                  ((dump_options.dump_path != ((void *)0))?dump_options.dump_path:
                   ((ctx->statedump_path != ((void *)0))?ctx->statedump_path:
                    DATADIR "/run/gluster")));
        ...
        if ((dump_options.dump_iobuf == _gf_true))
                iobuf_stats_dump (ctx->iobuf_pool);
        if ((dump_options.dump_callpool == _gf_true))
        ...
        i = 0;
        for (trav = ((typeof(*trav) *)((char *)((&ctx->graphs)->next)-(unsigned long)(&((typeof(*trav) *)0)->list))); &trav->list != (&ctx->graphs); trav = ((typeof(*trav) *)((char *)(trav->list.next)-(unsigned long)(&((typeof(*trav) *)0)->list)))) {
                if (trav == ctx->active)
                        continue;
...
out:
        ...
        __gf_free (dump_options.dump_path);
        dump_options.dump_path = ((void *)0);
        gf_proc_dump_unlock ();
...
gf_proc_dump_init ()
{
        pthread_mutex_init (&gf_proc_dump_mutex, ((void *)0));
