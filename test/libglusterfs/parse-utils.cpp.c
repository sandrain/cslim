...
{
        int rc = 0;
        struct parser *parser = ((void *)0);

        parser = __gf_malloc (sizeof(*parser), gf_common_mt_parser_t, "gf_common_mt_parser_t");
        if (!parser)
                goto out;
        ...
        rc = regcomp (&parser->preg, parser->regex, 1);
        if (rc != 0) {
                do { _gf_msg ("parse-utils",
 "./parse-utils.c"
# 57 "./parse-utils.c"
                , __FUNCTION__,
 58
# 57 "./parse-utils.c"
                , GF_LOG_INFO, 0, 0, (100000 + 1000 + 102), "Failed to compile regex pattern."); } while (0)
                                                           ;
                parser_deinit (parser);
                parser = ((void *)0);
                goto out;
        }
        parser->complete_str = ((void *)0);
out:
        return parser;
}
# 80 "./parse-utils.c"
...
parser_set_string (struct parser *parser, const char *complete_str)
{
        ...
        do { if (!parser) { (*__errno_location ()) = 22; do { _gf_msg ("parse-utils", "./parse-utils.c", __FUNCTION__, 85, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "parser"); } while (0); goto out; } } while (0);
        do { if (!complete_str) { (*__errno_location ()) = 22; do { _gf_msg ("parse-utils", "./parse-utils.c", __FUNCTION__, 86, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "complete_str"); } while (0); goto out; } } while (0);

        parser->complete_str = gf_strdup (complete_str);
        do { if (!(parser)) { (ret) = -12; do { _gf_msg ("parse-utils",
 "./parse-utils.c"
# 89 "./parse-utils.c"
        , __FUNCTION__,
 90
# 89 "./parse-utils.c"
        , GF_LOG_CRITICAL, 12, 0, (100000 + 1000 + 18), ("Failed to duplicate string!")); } while (0); goto out; } } while (0)
                                                                   ;
...
parser_unset_string (struct parser *parser)
{
        ...
        do { if (!parser) { (*__errno_location ()) = 22; do { _gf_msg ("parse-utils", "./parse-utils.c", __FUNCTION__, 114, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "parser"); } while (0); goto out; } } while (0);

        __gf_free (parser->complete_str);
        parser->complete_str = ((void *)0);
        ret = 0;
out:
        return ret;
...
parser_get_next_match (struct parser *parser)
{
        ...
        do { if (!parser) { (*__errno_location ()) = 22; do { _gf_msg ("parse-utils", "./parse-utils.c", __FUNCTION__, 158, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "parser"); } while (0); goto out; } } while (0);
...
# 162 "./parse-utils.c"
 ...
 164
# 162 "./parse-utils.c"
                , GF_LOG_DEBUG, 0, 0, 0, "Could not match %s with regex %s", parser->_rstr, parser->regex); } while (0)
        ...
        do { if (!(match)) { (rc) = -12; do { _gf_msg ("parse-utils",
 "./parse-utils.c"
# 171 "./parse-utils.c"
        , __FUNCTION__,
 172
# 171 "./parse-utils.c"
        , GF_LOG_CRITICAL, 12, 0, (100000 + 1000 + 18), ("Duplicating match failed!")); } while (0); goto out; } } while (0)
                                                                 ;
