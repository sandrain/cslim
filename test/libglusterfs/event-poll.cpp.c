...
# 55 "./event-poll.c"
 ...
 57
# 55 "./event-poll.c"
                        , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 12), "read on %d returned " "error", fd); } while (0)
...
__event_getindex (struct event_pool *event_pool, int fd, int idx)
{
        ...
        do { if (!event_pool) { (*__errno_location ()) = 22; do { _gf_msg ("event", "./event-poll.c", __FUNCTION__, 71, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "event_pool"); } while (0); goto out; } } while (0);
...
{
        struct event_pool *event_pool = ((void *)0);
        int ret = -1;

        event_pool = __gf_calloc (1, sizeof (*event_pool), gf_common_mt_event_pool, "gf_common_mt_event_pool")
                                                        ;
        ...
        event_pool->reg = __gf_calloc (event_pool->count, sizeof (*event_pool->reg), gf_common_mt_reg, "gf_common_mt_reg")

                                                      ;

        if (!event_pool->reg) {
                __gf_free (event_pool);
                return ((void *)0);
        }

        pthread_mutex_init (&event_pool->mutex, ((void *)0));
...
# 121 "./event-poll.c"
 ...
 122
# 121 "./event-poll.c"
                , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 56), "pipe creation failed"); } while (0)
                                               ;
                __gf_free (event_pool->reg);
                __gf_free (event_pool);
                return ((void *)0);
        }

        ret = fcntl (event_pool->breaker[0], 4, 04000);
        if (ret == -1) {
                do { _gf_msg ("poll",
 "./event-poll.c"
# 130 "./event-poll.c"
 ...
 131
# 130 "./event-poll.c"
                , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 57), "could not set pipe to non blocking mode"); } while (0)
                                                                  ;
        ...
        ret = fcntl (event_pool->breaker[1], 4, 04000);
        if (ret == -1) {
                do { _gf_msg ("poll",
 "./event-poll.c"
# 143 "./event-poll.c"
...
# 158 "./event-poll.c"
 ...
 159
# 158 "./event-poll.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 58), "could not register pipe fd with poll event loop"); } while (0)
                                                                          ;
                sys_close (event_pool->breaker[0]);
                sys_close (event_pool->breaker[1]);
                event_pool->breaker[0] = event_pool->breaker[1] = -1;
...
# 170 "./event-poll.c"
 ...
 173
# 170 "./event-poll.c"
                , GF_LOG_INFO, 0, 0, (100000 + 1000 + 59), "Currently poll " "does not use multiple event processing threads, " "thread count (%d) ignored", eventthreadcount); } while (0)
                     ...
                     void *data, int poll_in, int poll_out)
{
        ...
        do { if (!event_pool) { (*__errno_location ()) = 22; do { _gf_msg ("event", "./event-poll.c", __FUNCTION__, 187, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "event_pool"); } while (0); goto out; } } while (0);
        ...
        pthread_mutex_lock (&event_pool->mutex);
        {
                if (event_pool->count == event_pool->used)
                {
                        ...
                        event_pool->reg = __gf_realloc (event_pool->reg, event_pool->count * sizeof (*event_pool->reg))
                ...
                event_pool->reg[idx].events = 0x002;
                event_pool->reg[idx].handler = handler;
                event_pool->reg[idx].data = data;

                switch (poll_in) {
                case 1:
                        event_pool->reg[idx].events |= 0x001;
                        break;
                case 0:
                        event_pool->reg[idx].events &= ~0x001;
                        break;
...
# 220 "./event-poll.c"
 ...
 222
# 220 "./event-poll.c"
                        , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 185), "invalid poll_in value %d", poll_in); } while (0)
                ...
                switch (poll_out) {
                case 1:
                        event_pool->reg[idx].events |= 0x004;
                        break;
                case 0:
                        event_pool->reg[idx].events &= ~0x004;
                        break;
...
# 237 "./event-poll.c"
 ...
 239
# 237 "./event-poll.c"
                        , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 186), "invalid poll_out value %d", poll_out); } while (0)
...
event_unregister_poll (struct event_pool *event_pool, int fd, int idx_hint)
{
        ...
        do { if (!event_pool) { (*__errno_location ()) = 22; do { _gf_msg ("event", "./event-poll.c", __FUNCTION__, 259, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "event_pool"); } while (0); goto out; } } while (0);
...
# 266 "./event-poll.c"
 ...
 268
# 266 "./event-poll.c"
                        , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 60), "index not found for fd=%d (idx_hint=%d)", fd, idx_hint); } while (0)
                      ...
                      int poll_in, int poll_out)
{
        ...
        do { if (!event_pool) { (*__errno_location ()) = 22; do { _gf_msg ("event", "./event-poll.c", __FUNCTION__, 304, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "event_pool"); } while (0); goto out; } } while (0);
...
{
        event_handler_t handler = ((void *)0);
        void *data = ((void *)0);
        int idx = -1;
        int ret = 0;
...
# 376 "./event-poll.c"
 ...
 378
# 376 "./event-poll.c"
                        , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 60), "index not found for " "fd=%d (idx_hint=%d)", ufds[i].fd, i); } while (0)
...
unlock:
        ...
        if (handler)
                ret = handler (ufds[i].fd, idx, data,
                               (ufds[i].revents & (0x001|0x002)),
                               (ufds[i].revents & (0x004)),
                               (ufds[i].revents & (0x008|0x010|0x020)));
                            ...
                            struct pollfd *ufds, int size)
{
        ...
        pthread_mutex_lock (&event_pool->mutex);
        {
                ...
                if (event_pool->used > event_pool->evcache_size) {
                        __gf_free (event_pool->evcache);

                        event_pool->evcache = ufds = ((void *)0);

                        event_pool->evcache_size = event_pool->used;

                        ufds = __gf_calloc (sizeof (struct pollfd), event_pool->evcache_size, gf_common_mt_pollfd, "gf_common_mt_pollfd")
...
event_dispatch_poll (struct event_pool *event_pool)
{
        ...
        do { if (!event_pool) { (*__errno_location ()) = 22; do { _gf_msg ("event", "./event-poll.c", __FUNCTION__, 448, GF_LOG_ERROR, (*__errno_location ()), 1, (100000 + 1000 + 172), "invalid argument: " "event_pool"); } while (0); goto out; } } while (0);
