...
# 1 "./rbthash.h" 1
# 13 "./rbthash.h"
# 1 "/home/hyogi/workspace/glusterfs/contrib/rbtree/rb.h" 1
# 33 "/home/hyogi/workspace/glusterfs/contrib/rbtree/rb.h"
...
typedef int wchar_t;
# 34 "/home/hyogi/workspace/glusterfs/contrib/rbtree/rb.h" 2
...
{
        int ret = 0;
        rbthash_entry_t *e1 = ((void *)0);
        rbthash_entry_t *e2 = ((void *)0);
...
__rbthash_init_buckets (rbthash_table_t *tbl, int buckets)
{
        ...
        for (; i < buckets; i++) {
                pthread_mutex_init (&tbl->buckets[i].bucketlock, 0);
                tbl->buckets[i].bucket = rb_create ((rb_comparison_func *)rbthash_comparator, tbl, ((void *)0));
...
# 61 "./rbthash.c"
 ...
 63
# 61 "./rbthash.c"
                        , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 31), "Failed to " "create rb table bucket"); } while (0)
...
# 95 "./rbthash.c"
 ...
 96
# 95 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 25), "Hash function not given"); } while (0)
                                                  ;
                return ((void *)0);
        }
...
# 101 "./rbthash.c"
 ...
 102
# 101 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 2), "Both mem-pool and expected entries not provided"); } while (0)
                                                                          ;
                return ((void *)0);
        }
...
# 107 "./rbthash.c"
 ...
 108
# 107 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 2), "Both mem-pool and expected entries are provided"); } while (0)
                                                                          ;
                return ((void *)0);
        }


        newtab = __gf_calloc (1, sizeof (*newtab), gf_common_mt_rbthash_table_t, "gf_common_mt_rbthash_table_t")
                                                         ;
        if (!newtab)
                return ((void *)0);

        newtab->buckets = __gf_calloc (buckets, sizeof (struct rbthash_bucket), gf_common_mt_rbthash_bucket, "gf_common_mt_rbthash_bucket")
                                                                 ;
        if (!newtab->buckets) {
                goto free_newtab;
        }
        ...
        if (expected_entries) {
                newtab->entrypool =
                        mem_pool_new_fn (sizeof(rbthash_entry_t), expected_entries, "rbthash_entry_t");
                if (!newtab->entrypool) {
                        goto free_buckets;
                }
        ...
        pthread_mutex_init (&newtab->tablelock, 0);
        do { (&newtab->list)->next = (&newtab->list)->prev = &newtab->list; } while (0);
...
# 141 "./rbthash.c"
 ...
 143
# 141 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 149), "Failed to init buckets"); } while (0)
...
# 147 "./rbthash.c"
 ...
 148
# 147 "./rbthash.c"
                , GF_LOG_TRACE, 0, 0, 0, "Inited hash table: buckets:" " %d", buckets); } while (0)
                                       ;
        }
...
# 178 "./rbthash.c"
 ...
 180
# 178 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 195), "Failed to get entry from mem-pool"); } while (0)
        ...
        entry->data = data;
        entry->key = __gf_calloc (keylen, sizeof (char), gf_common_mt_char, "gf_common_mt_char");
        if (!entry->key) {
                goto free_entry;
        }
        ...
        do { _gf_msg ("rbthash", "./rbthash.c", __FUNCTION__, 194, GF_LOG_TRACE, 0, 0, 0, "HASH: %u", entry->keyhash); } while (0);
...
rbthash_deinit_entry (rbthash_table_t *tbl, rbthash_entry_t *entry)
{
        ...
        if (tbl) {
                ...
                pthread_mutex_lock (&tbl->tablelock);
                {
                        list_del_init (&entry->list);
                }
                pthread_mutex_unlock (&tbl->tablelock);
...
rbthash_entry_bucket (rbthash_table_t *tbl, rbthash_entry_t * entry)
{
        ...
        do { _gf_msg ("rbthash", "./rbthash.c", __FUNCTION__, 240, GF_LOG_TRACE, 0, 0, 0, "BUCKET: %d", nbucket); } while (0);
...
{
        struct rbthash_bucket *bucket = ((void *)0);
        int ret = -1;
...
# 256 "./rbthash.c"
 ...
 258
# 256 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 196), "Failed to get bucket"); } while (0)
        ...
        ret = 0;
        pthread_mutex_lock (&bucket->bucketlock);
        {
...
# 266 "./rbthash.c"
 ...
 268
# 266 "./rbthash.c"
                        , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 197), "Failed to insert" " entry"); } while (0)

                                         ;
                        ret = -1;
                }
        }
        pthread_mutex_unlock (&bucket->bucketlock);
...
# 290 "./rbthash.c"
 ...
 292
# 290 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 198), "Failed to init entry"); } while (0)
...
rbthash_key_bucket (rbthash_table_t *tbl, void *key, int keylen)
{
        ...
        do { _gf_msg ("rbthash", "./rbthash.c", __FUNCTION__, 325, GF_LOG_TRACE, 0, 0, 0, "HASH: %u", keyhash); } while (0);
        nbucket = (keyhash % tbl->numbuckets);
        do { _gf_msg ("rbthash", "./rbthash.c", __FUNCTION__, 327, GF_LOG_TRACE, 0, 0, 0, "BUCKET: %u", nbucket); } while (0);
...
# 345 "./rbthash.c"
 ...
 346
# 345 "./rbthash.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 26), "Failed to get bucket"); } while (0)
                                               ;
                return ((void *)0);
        }
...
rbthash_table_destroy_buckets (rbthash_table_t *tbl)
{
        ...
        for (;x < tbl->numbuckets; x++) {
                pthread_mutex_destroy (&tbl->buckets[x].bucketlock);
                        ...
                        void *mydata)
{
        ...
        if ((tbl == ((void *)0)) || (traverse == ((void *)0))) {
                goto out;
        }

        pthread_mutex_lock (&tbl->tablelock);
        {
                for (entry = ((typeof(*entry) *)((char *)((&tbl->list)->next)-(unsigned long)(&((typeof(*entry) *)0)->list))); &entry->list != (&tbl->list); entry = ((typeof(*entry) *)((char *)(entry->list.next)-(unsigned long)(&((typeof(*entry) *)0)->list)))) {
