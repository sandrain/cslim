...
# 22 "/usr/include/sys/uio.h" 2 3 4
...
gp_write_file (struct gf_printer *gp, char *buf, size_t len)
{
        ...
        if (fwrite (buf, len, 1, f) != 1) {
                do { _gf_msg ("graph-print",
 "./graph-print.c"
# 31 "./graph-print.c"
                , __FUNCTION__,
 32
# 31 "./graph-print.c"
                , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 51), "fwrite failed"); } while (0)
                                                              ;
...
# 46 "./graph-print.c"
 ...
 47
# 46 "./graph-print.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 50), "buffer full"); } while (0)
                                      ;
                           ...
                           void *tmp)
{
        ...
        int ret = 0;
        do { ret = gpprintf (gp, "    option %s %s\n", k, v->data); if (ret == -1) goto out; else gp->len += ret; } while (0);
        return 0;
out:
...
glusterfs_graph_print (struct gf_printer *gp, glusterfs_graph_t *graph)
{
        ...
        for (; trav; trav = trav->prev) {
                do { ret = gpprintf (gp, "volume %s\n    type %s\n", trav->name, trav->type); if (ret == -1) goto out; else gp->len += ret; } while (0)
                                     ;
                ...
                if (trav->children) {
                        do { ret = gpprintf (gp, "    subvolumes"); if (ret == -1) goto out; else gp->len += ret; } while (0);

                        for (xch = trav->children; xch; xch = xch->next)
                                do { ret = gpprintf (gp, " %s", xch->xlator->name); if (ret == -1) goto out; else gp->len += ret; } while (0);

                        do { ret = gpprintf (gp, "\n"); if (ret == -1) goto out; else gp->len += ret; } while (0);
                }

                do { ret = gpprintf (gp, "end-volume\n"); if (ret == -1) goto out; else gp->len += ret; } while (0);
                if (trav != graph->first)
                        do { ret = gpprintf (gp, "\n"); if (ret == -1) goto out; else gp->len += ret; } while (0);
        }
...
# 138 "./graph-print.c"
 ...
 139
# 138 "./graph-print.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 52), "printing failed"); } while (0)
                                          ;
...
# 172 "./graph-print.c"
 ...
 173
# 172 "./graph-print.c"
                , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 10), "cannot open /dev/null"); } while (0)
                                                                      ;
        ...
        buf = __gf_calloc (1, len + 1, gf_common_mt_graph_buf, "gf_common_mt_graph_buf");
        if (!buf) {
                return ((void *)0);
        }
        iov.iov_base = buf;
