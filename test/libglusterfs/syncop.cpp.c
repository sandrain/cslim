...
syncop_create_frame (xlator_t *this)
{
 ...
 if (opctx && (opctx->valid & 0x00000008))
  frame->root->pid = opctx->pid;
 else
  frame->root->pid = getpid ();

 if (opctx && (opctx->valid & 0x00000001))
  frame->root->uid = opctx->uid;
 else
  frame->root->uid = geteuid ();

 if (opctx && (opctx->valid & 0x00000002))
  frame->root->gid = opctx->gid;
 else
  frame->root->gid = getegid ();

 if (opctx && (opctx->valid & 0x00000004)) {
  ngrps = opctx->ngrps;

  if (ngrps != 0 && opctx->groups != ((void *)0)) {
  ...
  if (getgroups (ngrps, frame->root->groups) < 0) {
   ...
   return ((void *)0);
        ...
        if (opctx && (opctx->valid & 0x00000010))
...
syncopctx_setfsuid (void *uid)
{
 ...
 if (!opctx) {
  opctx = __gf_calloc (1, sizeof (*opctx), gf_common_mt_syncopctx, "gf_common_mt_syncopctx");
  if (!opctx) {
   ret = -1;
   goto out;
  }
...
syncopctx_setfsgroups (int count, const void *groups)
{
 ...
 if (count && opctx->grpsize < count) {
  if (opctx->groups) {
   tmpgroups = __gf_realloc (opctx->groups, (sizeof (gid_t) * count))
                               ;
   ...
   tmpgroups = __gf_calloc (count, sizeof (gid_t), gf_common_mt_syncopctx, "gf_common_mt_syncopctx")
                                   ;
   if (tmpgroups == ((void *)0)) {
    opctx->grpsize = 0;
    ret = -1;
...
__run (struct synctask *task)
{
        ...
        case SYNCTASK_SUSPEND:
                break;
        case SYNCTASK_RUN:
                do { _gf_msg (task->xl->name,
 "./syncop.c"
# 261 "./syncop.c"
                , __FUNCTION__,
 262
# 261 "./syncop.c"
                , GF_LOG_DEBUG, 0, 0, 0, "re-running already running" " task"); } while (0)
                                      ;
                env->runcount--;
                break;
        case SYNCTASK_WAIT:
...
# 269 "./syncop.c"
 ...
 270
# 269 "./syncop.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 133), "running completed task"); } while (0)
                                                                        ;
  return;
...
# 273 "./syncop.c"
 ...
 275
# 273 "./syncop.c"
  , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 134), "attempted to wake up " "zombie!!"); } while (0)
...
# 301 "./syncop.c"
 ...
 303
# 301 "./syncop.c"
                , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 135), "re-waiting already waiting " "task"); } while (0)
...
# 312 "./syncop.c"
 ...
 314
# 312 "./syncop.c"
  , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 136), "attempted to sleep a zombie!!"); } while (0)
...
{
        xlator_t *oldTHIS = (*__glusterfs_this_location());
...
# 339 "./syncop.c"
 ...
 340
# 339 "./syncop.c"
                , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 137), "swapcontext failed"); } while (0)
                                                                        ;
        }

        (*__glusterfs_this_location()) = oldTHIS;
}
...
synctask_destroy (struct synctask *task)
{
        ...
        __gf_free (task->stack);

        if (task->opframe)
                STACK_DESTROY (task->opframe->root);

        if (task->synccbk == ((void *)0)) {
...
{
        struct synctask *newtask = ((void *)0);
        xlator_t *this = (*__glusterfs_this_location());
        int destroymode = 0;

        do { if (!env) { (*__errno_location ()) = 22; do { _gf_msg ((this ? (this->name) : "(Govinda! Govinda!)"), "./syncop.c", __FUNCTION__, 447, GF_LOG_WARNING, 22, 1, (100000 + 1000 + 172), "invalid argument: " "env"); } while (0); goto err; } } while (0);
        do { if (!fn) { (*__errno_location ()) = 22; do { _gf_msg ((this ? (this->name) : "(Govinda! Govinda!)"), "./syncop.c", __FUNCTION__, 448, GF_LOG_WARNING, 22, 1, (100000 + 1000 + 172), "invalid argument: " "fn"); } while (0); goto err; } } while (0);
        ...
        newtask = __gf_calloc (1, sizeof (*newtask), gf_common_mt_synctask, "gf_common_mt_synctask");
        if (!newtask)
                return ((void *)0);
        ...
        do { (&newtask->all_tasks)->next = (&newtask->all_tasks)->prev = &newtask->all_tasks; } while (0);
        do { (&newtask->waitq)->next = (&newtask->waitq)->prev = &newtask->waitq; } while (0);
...
# 491 "./syncop.c"
 ...
 492
# 491 "./syncop.c"
                , GF_LOG_ERROR, (*__errno_location ()), 0, (100000 + 1000 + 113), "getcontext failed"); } while (0)
                                                                      ;
                goto err;
        }

        if (stacksize <= 0) {
                newtask->stack = __gf_calloc (1, env->stacksize, gf_common_mt_syncstack, "gf_common_mt_syncstack")
                                                                   ;
                newtask->ctx.uc_stack.ss_size = env->stacksize;
        } else {
                newtask->stack = __gf_calloc (1, stacksize, gf_common_mt_syncstack, "gf_common_mt_syncstack")
                                                                   ;
        ...
        if (!cbk) {
                pthread_mutex_init (&newtask->mutex, ((void *)0));
                pthread_cond_init (&newtask->cond, ((void *)0));
                newtask->done = 0;
        }
...
# 620 "./syncop.c"
                ...
                task = ((struct synctask *)((char *)(env->runq.next)-(unsigned long)(&((struct synctask *)0)->all_tasks)));
...
synctask_switchto (struct synctask *task)
{
        ...
        synctask_set (task);
        (*__glusterfs_this_location()) = task->xl;
...
syncenv_scale (struct syncenv *env)
{
        ...
        pthread_mutex_lock (&env->mutex);
        {
                ...
                while (diff) {
                        ...
                        env->proc[i].env = env;
                        ret = gf_thread_create (&env->proc[i].processor, ((void *)0),
...
{
        struct syncenv *newenv = ((void *)0);
        int ret = 0;
        int i = 0;

 if (!procmin || procmin < 0)
  procmin = 2;
 if (!procmax || procmax > 16)
  procmax = 16;

 if (procmin > procmax)
  return ((void *)0);

        newenv = __gf_calloc (1, sizeof (*newenv), gf_common_mt_syncenv, "gf_common_mt_syncenv");

        if (!newenv)
                return ((void *)0);

        pthread_mutex_init (&newenv->mutex, ((void *)0));
        pthread_cond_init (&newenv->cond, ((void *)0));

        do { (&newenv->runq)->next = (&newenv->runq)->prev = &newenv->runq; } while (0);
        do { (&newenv->waitq)->next = (&newenv->waitq)->prev = &newenv->waitq; } while (0);

        newenv->stacksize = (2 * 1024 * 1024);
        if (stacksize)
 ...
 newenv->procmax = procmax;
        ...
        for (i = 0; i < newenv->procmin; i++) {
                ...
                ret = gf_thread_create (&newenv->proc[i].processor, ((void *)0),
...
synclock_init (synclock_t *lock, lock_attr_t attr)
{
        ...
        lock->lock = 0;
        lock->attr = attr;
        do { (&lock->waitq)->next = (&lock->waitq)->prev = &lock->waitq; } while (0);
...
# 889 "./syncop.c"
 ...
 891
# 889 "./syncop.c"
                                , GF_LOG_TRACE, 0, 0, 0, "Recursive lock called by" " sync task.owner= %p,lock=%d", lock->owner, lock->lock); } while (0)
...
# 898 "./syncop.c"
 ...
 901
# 898 "./syncop.c"
                                , GF_LOG_TRACE, 0, 0, 0, "Recursive lock called by" " thread ,owner=%u lock=%d", (unsigned int) lock->owner_tid, lock->lock); } while (0)
...
# 906 "./syncop.c"
 ...
 907
# 906 "./syncop.c"
                        , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 164), "unknown lock type"); } while (0)
                                                                              ;
                        break;
                }
        }
...
# 992 "./syncop.c"
 ...
 993
# 992 "./syncop.c"
                , GF_LOG_CRITICAL, 0, 0, (100000 + 1000 + 165), "Unlock called  before lock "); } while (0)
                                                      ;
                return -1;
        }
        curr = synctask_get ();
...
# 1005 "./syncop.c"
 ...
 1006
# 1005 "./syncop.c"
                        , GF_LOG_TRACE, 0, 0, 0, "Unlock success %p, remaining" " locks=%d", lock->owner, lock->lock); } while (0)
                                                                           ;
                } else {
                        do { _gf_msg ("",
...
# 1008 "./syncop.c"
 ...
 1010
# 1008 "./syncop.c"
                        , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 166), "Unlock called by %p, but lock held by %p", curr, lock->owner); } while (0)
...
# 1017 "./syncop.c"
 ...
 1020
# 1017 "./syncop.c"
                        , GF_LOG_TRACE, 0, 0, 0, "Unlock success %u, remaining " "locks=%d", (unsigned int)lock->owner_tid, lock->lock); } while (0)
...
# 1022 "./syncop.c"
 ...
 1025
# 1022 "./syncop.c"
                        , GF_LOG_WARNING, 0, 0, (100000 + 1000 + 166), "Unlock called by %u, but lock held by %u", (unsigned int) pthread_self(), (unsigned int) lock->owner_tid); } while (0)
...
# 1049 "./syncop.c"
        ...
        if (!list_empty (&lock->waitq)) {
                task = ((struct synctask *)((char *)(lock->waitq.next)-(unsigned long)(&((struct synctask *)0)->waitq)));
...
syncbarrier_init (struct syncbarrier *barrier)
{
 ...
 barrier->count = 0;
 do { (&barrier->waitq)->next = (&barrier->waitq)->prev = &barrier->waitq; } while (0);
...
__syncbarrier_wake (struct syncbarrier *barrier)
{
 ...
 if (!list_empty (&barrier->waitq)) {
  task = ((struct synctask *)((char *)(barrier->waitq.next)-(unsigned long)(&((struct synctask *)0)->waitq)));
                list_del_init (&task->waitq);
  synctask_wake (task);
 }
                   ...
                   struct iatt *iatt, dict_t *xdata, struct iatt *parent)
{
        ...
        do { if (args->task) { synctask_wake (args->task); } else { pthread_mutex_lock (&args->mutex); { args->done = 1; pthread_cond_signal (&args->cond); } pthread_mutex_unlock (&args->mutex); } } while (0);
               ...
               struct iatt *parent, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->lookup_cbk) tmp_cbk = syncop_lookup_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->lookup"; _new->unwind_to = "syncop_lookup_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->lookup_cbk = syncop_lookup_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->lookup); subvol->fops->lookup (_new, subvol, loc, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                              ;
...
{
        struct syncargs *args = ((void *)0);
        gf_dirent_t *entry = ((void *)0);
        gf_dirent_t *tmp = ((void *)0);

        int count = 0;

        args = cookie;

        do { (&args->entries.list)->next = (&args->entries.list)->prev = &args->entries.list; } while (0);
        ...
        if (op_ret >= 0) {
                for (entry = ((typeof(*entry) *)((char *)((&entries->list)->next)-(unsigned long)(&((typeof(*entry) *)0)->list))); &entry->list != (&entries->list); entry = ((typeof(*entry) *)((char *)(entry->list.next)-(unsigned long)(&((typeof(*entry) *)0)->list)))) {
...
# 1273 "./syncop.c"
 ...
 1274
# 1273 "./syncop.c"
                        , GF_LOG_TRACE, 0, 0, 0, "adding entry=%s, " "count=%d", tmp->d_name, count); } while (0)
                                                                     ;
                 ...
                 dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->readdirp_cbk) tmp_cbk = syncop_readdirp_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->readdirp"; _new->unwind_to = "syncop_readdirp_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->readdirp_cbk = syncop_readdirp_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->readdirp); subvol->fops->readdirp (_new, subvol, fd, size, off, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                        ;
                ...
                dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->readdir_cbk) tmp_cbk = syncop_readdir_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->readdir"; _new->unwind_to = "syncop_readdir_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->readdir_cbk = syncop_readdir_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->readdir); subvol->fops->readdir (_new, subvol, fd, size, off, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                        ;
                ...
                dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->opendir_cbk) tmp_cbk = syncop_opendir_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->opendir"; _new->unwind_to = "syncop_opendir_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->opendir_cbk = syncop_opendir_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->opendir); subvol->fops->opendir (_new, subvol, loc, fd, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                  ;
                 ...
                 dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fsyncdir_cbk) tmp_cbk = syncop_fsyncdir_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fsyncdir"; _new->unwind_to = "syncop_fsyncdir_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fsyncdir_cbk = syncop_fsyncdir_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fsyncdir); subvol->fops->fsyncdir (_new, subvol, fd, datasync, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                       ;
                    ...
                    dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->removexattr_cbk) tmp_cbk = syncop_removexattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->removexattr"; _new->unwind_to = "syncop_removexattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->removexattr_cbk = syncop_removexattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->removexattr); subvol->fops->removexattr (_new, subvol, loc, name, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                                               ;
                     ...
                     dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fremovexattr_cbk) tmp_cbk = syncop_fremovexattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fremovexattr"; _new->unwind_to = "syncop_fremovexattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fremovexattr_cbk = syncop_fremovexattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fremovexattr); subvol->fops->fremovexattr (_new, subvol, fd, name, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                                               ;
   ...
   dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->setxattr_cbk) tmp_cbk = syncop_setxattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->setxattr"; _new->unwind_to = "syncop_setxattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->setxattr_cbk = syncop_setxattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->setxattr); subvol->fops->setxattr (_new, subvol, loc, dict, flags, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                           ;
    ...
    dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fsetxattr_cbk) tmp_cbk = syncop_fsetxattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fsetxattr"; _new->unwind_to = "syncop_fsetxattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fsetxattr_cbk = syncop_fsetxattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fsetxattr); subvol->fops->fsetxattr (_new, subvol, fd, dict, flags, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                          ;
                  ...
                  dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->getxattr_cbk) tmp_cbk = syncop_getxattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->getxattr"; _new->unwind_to = "syncop_getxattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->getxattr_cbk = syncop_getxattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->getxattr); subvol->fops->getxattr (_new, subvol, loc, ((void *)0), xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                    ;
                 ...
                 dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->getxattr_cbk) tmp_cbk = syncop_getxattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->getxattr"; _new->unwind_to = "syncop_getxattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->getxattr_cbk = syncop_getxattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->getxattr); subvol->fops->getxattr (_new, subvol, loc, key, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                   ;
                  ...
                  dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fgetxattr_cbk) tmp_cbk = syncop_getxattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fgetxattr"; _new->unwind_to = "syncop_getxattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fgetxattr_cbk = syncop_getxattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fgetxattr); subvol->fops->fgetxattr (_new, subvol, fd, key, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                  ;
...

{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->statfs_cbk) tmp_cbk = syncop_statfs_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->statfs"; _new->unwind_to = "syncop_statfs_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->statfs_cbk = syncop_statfs_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->statfs); subvol->fops->statfs (_new, subvol, loc, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                              ;
                ...
                dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->setattr_cbk) tmp_cbk = syncop_setattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->setattr"; _new->unwind_to = "syncop_setattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->setattr_cbk = syncop_setattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->setattr); subvol->fops->setattr (_new, subvol, loc, iatt, valid, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                           ;
                 ...
                 dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fsetattr_cbk) tmp_cbk = syncop_setattr_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fsetattr"; _new->unwind_to = "syncop_setattr_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fsetattr_cbk = syncop_setattr_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fsetattr); subvol->fops->fsetattr (_new, subvol, fd, iatt, valid, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                          ;
      ...
      dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->open_cbk) tmp_cbk = syncop_open_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->open"; _new->unwind_to = "syncop_open_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->open_cbk = syncop_open_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->open); subvol->fops->open (_new, subvol, loc, flags, fd, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                         ;
              ...
              struct iobref **iobref, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->readv_cbk) tmp_cbk = syncop_readv_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->readv"; _new->unwind_to = "syncop_readv_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->readv_cbk = syncop_readv_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->readv); subvol->fops->readv (_new, subvol, fd, size, off, flags, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                               ;
               ...
               uint32_t flags, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->writev_cbk) tmp_cbk = syncop_writev_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->writev"; _new->unwind_to = "syncop_writev_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->writev_cbk = syncop_writev_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->writev); subvol->fops->writev (_new, subvol, fd, (struct iovec *) vector, count, offset, flags, iobref, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                  ...
                  dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->writev_cbk) tmp_cbk = syncop_writev_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->writev"; _new->unwind_to = "syncop_writev_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->writev_cbk = syncop_writev_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->writev); subvol->fops->writev (_new, subvol, fd, &vec, 1, offset, flags, iobref, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                                             ;
               ...
               dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->create_cbk) tmp_cbk = syncop_create_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->create"; _new->unwind_to = "syncop_create_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->create_cbk = syncop_create_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->create); subvol->fops->create (_new, subvol, loc, flags, mode, 0, fd, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                                  ;
               ...
               dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->unlink_cbk) tmp_cbk = syncop_unlink_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->unlink"; _new->unwind_to = "syncop_unlink_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->unlink_cbk = syncop_unlink_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->unlink); subvol->fops->unlink (_new, subvol, loc, 0, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                            ;
              ...
              dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->rmdir_cbk) tmp_cbk = syncop_rmdir_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->rmdir"; _new->unwind_to = "syncop_rmdir_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->rmdir_cbk = syncop_rmdir_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->rmdir); subvol->fops->rmdir (_new, subvol, loc, flags, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                ;
             ...
             dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->link_cbk) tmp_cbk = syncop_link_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->link"; _new->unwind_to = "syncop_link_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->link_cbk = syncop_link_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->link); subvol->fops->link (_new, subvol, oldloc, newloc, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                         ;
               ...
               dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->rename_cbk) tmp_cbk = syncop_rename_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->rename"; _new->unwind_to = "syncop_rename_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->rename_cbk = syncop_rename_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->rename); subvol->fops->rename (_new, subvol, oldloc, newloc, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                         ;
                  ...
                  dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->ftruncate_cbk) tmp_cbk = syncop_ftruncate_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->ftruncate"; _new->unwind_to = "syncop_ftruncate_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->ftruncate_cbk = syncop_ftruncate_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->ftruncate); subvol->fops->ftruncate (_new, subvol, fd, offset, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                     ;
                 ...
                 dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->truncate_cbk) tmp_cbk = syncop_ftruncate_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->truncate"; _new->unwind_to = "syncop_ftruncate_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->truncate_cbk = syncop_ftruncate_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->truncate); subvol->fops->truncate (_new, subvol, loc, offset, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                      ;
              ...
              dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fsync_cbk) tmp_cbk = syncop_fsync_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fsync"; _new->unwind_to = "syncop_fsync_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fsync_cbk = syncop_fsync_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fsync); subvol->fops->fsync (_new, subvol, fd, dataonly, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                       ;
              ...
              dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->flush_cbk) tmp_cbk = syncop_flush_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->flush"; _new->unwind_to = "syncop_flush_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->flush_cbk = syncop_flush_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->flush); subvol->fops->flush (_new, subvol, fd, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                             ;
              ...
              dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fstat_cbk) tmp_cbk = syncop_fstat_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fstat"; _new->unwind_to = "syncop_fstat_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fstat_cbk = syncop_fstat_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fstat); subvol->fops->fstat (_new, subvol, fd, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                             ;
             ...
             dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->stat_cbk) tmp_cbk = syncop_fstat_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->stat"; _new->unwind_to = "syncop_fstat_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->stat_cbk = syncop_fstat_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->stat); subvol->fops->stat (_new, subvol, loc, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                              ;
                ...
                struct iatt *iatt, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->symlink_cbk) tmp_cbk = syncop_symlink_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->symlink"; _new->unwind_to = "syncop_symlink_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->symlink_cbk = syncop_symlink_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->symlink); subvol->fops->symlink (_new, subvol, newpath, loc, 0, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                          ;
   ...
   dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->readlink_cbk) tmp_cbk = syncop_readlink_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->readlink"; _new->unwind_to = "syncop_readlink_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->readlink_cbk = syncop_readlink_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->readlink); subvol->fops->readlink (_new, subvol, loc, size, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                    ;

        if (buffer)
                *buffer = args.buffer;
        else __gf_free (args.buffer);
              ...
              struct iatt *iatt, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->mknod_cbk) tmp_cbk = syncop_mknod_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->mknod"; _new->unwind_to = "syncop_mknod_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->mknod_cbk = syncop_mknod_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->mknod); subvol->fops->mknod (_new, subvol, loc, mode, rdev, 0, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                             ;
              ...
              dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->mkdir_cbk) tmp_cbk = syncop_mkdir_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->mkdir"; _new->unwind_to = "syncop_mkdir_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->mkdir_cbk = syncop_mkdir_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->mkdir); subvol->fops->mkdir (_new, subvol, loc, mode, 0, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                       ;
               ...
               dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->access_cbk) tmp_cbk = syncop_access_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->access"; _new->unwind_to = "syncop_access_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->access_cbk = syncop_access_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->access); subvol->fops->access (_new, subvol, loc, mask, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                    ;
   ...
   size_t len, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fallocate_cbk) tmp_cbk = syncop_fallocate_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fallocate"; _new->unwind_to = "syncop_fallocate_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fallocate_cbk = syncop_fallocate_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fallocate); subvol->fops->fallocate (_new, subvol, fd, keep_size, offset, len, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                                     ;
        ...
        dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->discard_cbk) tmp_cbk = syncop_discard_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->discard"; _new->unwind_to = "syncop_discard_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->discard_cbk = syncop_discard_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->discard); subvol->fops->discard (_new, subvol, fd, offset, len, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                          ;
  ...
  dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->zerofill_cbk) tmp_cbk = syncop_zerofill_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->zerofill"; _new->unwind_to = "syncop_zerofill_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->zerofill_cbk = syncop_zerofill_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->zerofill); subvol->fops->zerofill (_new, subvol, fd, offset, len, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                          ;
...
syncop_ipc (xlator_t *subvol, int32_t op, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->ipc_cbk) tmp_cbk = syncop_ipc_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->ipc"; _new->unwind_to = "syncop_ipc_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->ipc_cbk = syncop_ipc_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->ipc); subvol->fops->ipc (_new, subvol, op, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                             ;
             ...
             dict_t *xdata_in, off_t *off)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->seek_cbk) tmp_cbk = syncop_seek_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->seek"; _new->unwind_to = "syncop_seek_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->seek_cbk = syncop_seek_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->seek); subvol->fops->seek (_new, subvol, fd, offset, what, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                       ;
    ...
    dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->lk_cbk) tmp_cbk = syncop_lk_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->lk"; _new->unwind_to = "syncop_lk_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->lk_cbk = syncop_lk_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->lk); subvol->fops->lk (_new, subvol, fd, cmd, flock, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                         ;
                ...
                struct gf_flock *lock, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->inodelk_cbk) tmp_cbk = syncop_inodelk_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->inodelk"; _new->unwind_to = "syncop_inodelk_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->inodelk_cbk = syncop_inodelk_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->inodelk); subvol->fops->inodelk (_new, subvol, volume, loc, cmd, lock, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                                 ;
                ...
                dict_t *dict, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->xattrop_cbk) tmp_cbk = syncop_xattrop_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->xattrop"; _new->unwind_to = "syncop_xattrop_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->xattrop_cbk = syncop_xattrop_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->xattrop); subvol->fops->xattrop (_new, subvol, loc, flags, dict, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                           ;
                 ...
                 dict_t *dict, dict_t *xdata_in, dict_t **xdata_out)
{
        ...
        do { struct synctask *task = ((void *)0); call_frame_t *frame = ((void *)0); task = synctask_get (); (&args)->task = task; if (task) frame = task->opframe; else frame = syncop_create_frame ((*__glusterfs_this_location())); if (task) { frame->root->uid = task->uid; frame->root->gid = task->gid; } do { (&args)->task = synctask_get (); if ((&args)->task) break; pthread_mutex_init (&(&args)->mutex, ((void *)0)); pthread_cond_init (&(&args)->cond, ((void *)0)); (&args)->done = 0; } while (0); do { call_frame_t *_new = ((void *)0); xlator_t *old_THIS = ((void *)0); _new = mem_get0 (frame->root->pool->frame_mem_pool); if (!_new) { break; } typeof(subvol->fops->fxattrop_cbk) tmp_cbk = syncop_xattrop_cbk; _new->root = frame->root; _new->this = subvol; _new->ret = (ret_fn_t) tmp_cbk; _new->parent = frame; _new->cookie = (void *)(&args); _new->wind_from = __FUNCTION__; _new->wind_to = "subvol->fops->fxattrop"; _new->unwind_to = "syncop_xattrop_cbk"; pthread_mutex_init (&_new->lock, 0); pthread_mutex_lock (&frame->root->stack_lock); { list_add (&_new->frames, &frame->root->myframes); frame->ref_count++; } pthread_mutex_unlock (&frame->root->stack_lock); subvol->fops->fxattrop_cbk = syncop_xattrop_cbk; old_THIS = (*__glusterfs_this_location()); (*__glusterfs_this_location()) = subvol; if (subvol->ctx->measure_latency) gf_latency_begin (_new, subvol->fops->fxattrop); subvol->fops->fxattrop (_new, subvol, fd, flags, dict, xdata_in); (*__glusterfs_this_location()) = old_THIS; } while (0); do { if ((&args)->task) { synctask_yield ((&args)->task); } else { pthread_mutex_lock (&(&args)->mutex); { while (!(&args)->done) pthread_cond_wait (&(&args)->cond, &(&args)->mutex); } pthread_mutex_unlock (&(&args)->mutex); pthread_mutex_destroy (&(&args)->mutex); pthread_cond_destroy (&(&args)->cond); } } while (0); if (task) STACK_RESET (frame->root); else STACK_DESTROY (frame->root); } while (0)
                                          ;
