...
strfd_open ()
{
        strfd_t *strfd = ((void *)0);

        strfd = __gf_calloc (1, sizeof(*strfd), gf_common_mt_strfd_t, "gf_common_mt_strfd_t");
...
strvprintf (strfd_t *strfd, const char *fmt, va_list ap)
{
        ...
        if (!strfd->alloc_size) {
                strfd->data = __gf_calloc (((size + 1)>(4096)?(size + 1):(4096)), 1, gf_common_mt_strfd_data_t, "gf_common_mt_strfd_data_t")
                                                                   ;
                if (!strfd->data) {
                        free (str);
                        return -1;
                }
                strfd->alloc_size = ((size + 1)>(4096)?(size + 1):(4096));
        }

        if (strfd->alloc_size <= (strfd->size + size)) {
                char *tmp_ptr = ((void *)0);
                int new_size = (((strfd->alloc_size * 2))>(gf_roundup_next_power_of_two (strfd->size + size + 1))?((strfd->alloc_size * 2)):(gf_roundup_next_power_of_two (strfd->size + size + 1)))
                                                                                          ;
                tmp_ptr = __gf_realloc (strfd->data, new_size);
                if (!tmp_ptr) {
                        free (str);
                        return -1;
                }
