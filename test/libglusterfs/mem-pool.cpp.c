...
gf_mem_acct_enable_set (void *data)
{
        ...
        do { if (!(ctx != ((void *)0))) { do { _gf_msg ("", "./mem-pool.c", __FUNCTION__, 39, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "ctx != NULL"); } while (0); } } while (0);
        ...
        uint32_t type, const char *typestr)
{
        ...
        void *ptr = ((void *)0);
        struct mem_header *header = ((void *)0);
        ...
        do { if (!(xl != ((void *)0))) { do { _gf_msg ("", "./mem-pool.c", __FUNCTION__, 61, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "xl != NULL"); } while (0); } } while (0);

        do { if (!(xl->mem_acct != ((void *)0))) { do { _gf_msg ("", "./mem-pool.c", __FUNCTION__, 63, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "xl->mem_acct != NULL"); } while (0); } } while (0);

        do { if (!(type <= xl->mem_acct->num_types)) { do { _gf_msg ("", "./mem-pool.c", __FUNCTION__, 65, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "type <= xl->mem_acct->num_types"); } while (0); } } while (0);

        pthread_mutex_lock (&xl->mem_acct->rec[type].lock);
        {
  if (!xl->mem_acct->rec[type].typestr)
   xl->mem_acct->rec[type].typestr = typestr;
                ...
                xl->mem_acct->rec[type].max_size =
                        ((xl->mem_acct->rec[type].max_size)>(xl->mem_acct->rec[type].size)?(xl->mem_acct->rec[type].max_size):(xl->mem_acct->rec[type].size))
                                                          ;
                xl->mem_acct->rec[type].max_num_allocs =
                        ((xl->mem_acct->rec[type].max_num_allocs)>(xl->mem_acct->rec[type].num_allocs)?(xl->mem_acct->rec[type].max_num_allocs):(xl->mem_acct->rec[type].num_allocs))
                                                                ;
        }
        pthread_mutex_unlock (&xl->mem_acct->rec[type].lock);

        __sync_add_and_fetch(&xl->mem_acct->refcnt, 1);
...
__gf_calloc (size_t nmemb, size_t size, uint32_t type, const char *typestr)
{
        ...
        if (!(*__glusterfs_this_location())->ctx->mem_acct_enable)
                return __gf_default_calloc(nmemb,size);

        xl = (*__glusterfs_this_location());

        req_size = nmemb * size;
        tot_size = req_size + (sizeof (struct mem_header)) + 8;

        ptr = calloc (1, tot_size);

        if (!ptr) {
                do { _gf_msg_nomem ("", "./mem-pool.c", __FUNCTION__, 120, GF_LOG_ALERT, tot_size); } while (0);
                return ((void *)0);
        }
...
__gf_malloc (size_t size, uint32_t type, const char *typestr)
{
        ...
        if (!(*__glusterfs_this_location())->ctx->mem_acct_enable)
                return __gf_default_malloc(size);

        xl = (*__glusterfs_this_location());

        tot_size = size + (sizeof (struct mem_header)) + 8;

        ptr = malloc (tot_size);
        if (!ptr) {
                do { _gf_msg_nomem ("", "./mem-pool.c", __FUNCTION__, 144, GF_LOG_ALERT, tot_size); } while (0);
                return ((void *)0);
        }
...
__gf_realloc (void *ptr, size_t size)
{
        size_t tot_size = 0;
        char *new_ptr;
        struct mem_header *old_header = ((void *)0);
        struct mem_header *new_header = ((void *)0);
        struct mem_header tmp_header;

        if (!(*__glusterfs_this_location())->ctx->mem_acct_enable)
                return __gf_default_realloc(ptr,size);

        REQUIRE(((void *)0) != ptr);

        old_header = (struct mem_header *) (ptr - (sizeof (struct mem_header)));
        do { if (!(old_header->magic == 0xCAFEBABE)) { do { _gf_msg ("", "./mem-pool.c", __FUNCTION__, 167, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "old_header->magic == GF_MEM_HEADER_MAGIC"); } while (0); } } while (0);
        tmp_header = *old_header;

        tot_size = size + (sizeof (struct mem_header)) + 8;
        new_ptr = realloc (old_header, tot_size);
        if (!new_ptr) {
                do { _gf_msg_nomem ("", "./mem-pool.c", __FUNCTION__, 173, GF_LOG_ALERT, tot_size); } while (0);
                return ((void *)0);
        }
# 195 "./mem-pool.c"
        ...
        *(uint32_t *) (new_ptr + size) = 0xBAADF00D;
...
gf_vasprintf (char **string_ptr, const char *format, va_list arg)
{
        ...
        __builtin_va_copy(arg_save,arg);

        size = vsnprintf (((void *)0), 0, format, arg);
        size++;
        str = __gf_malloc (size, gf_common_mt_asprintf, "gf_common_mt_asprintf");
        if (str == ((void *)0)) {
...
__gf_free (void *free_ptr)
{
        ...
        if (!(*__glusterfs_this_location())->ctx->mem_acct_enable) {
                do { if (free_ptr != ((void *)0)) { free ((void *)free_ptr); free_ptr = (void *)0xeeeeeeee; } } while (0);
                return;
        }
        ...
        ptr = free_ptr - (sizeof (struct mem_header));
        header = (struct mem_header *) ptr;


        do { if (!(0xCAFEBABE == header->magic)) { do { _gf_msg ("", "./mem-pool.c", __FUNCTION__, 305, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "GF_MEM_HEADER_MAGIC == header->magic"); } while (0); } } while (0);
        ...
        do { if (!(0xBAADF00D == *(uint32_t *)((char *)free_ptr + header->size))) { do { _gf_msg ("",
 "./mem-pool.c"
# 313 "./mem-pool.c"
        , __FUNCTION__,
 314
# 313 "./mem-pool.c"
        , GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "GF_MEM_TRAILER_MAGIC == *(uint32_t *)((char *)free_ptr + header->size)"); } while (0); } } while (0)
                                                               ;

        pthread_mutex_lock (&mem_acct->rec[header->type].lock);
        {
                ...
                if (!mem_acct->rec[header->type].num_allocs)
                        mem_acct->rec[header->type].typestr = ((void *)0);
        }
        pthread_mutex_unlock (&mem_acct->rec[header->type].lock);
...
free:
        ...
        do { if (ptr != ((void *)0)) { free ((void *)ptr); ptr = (void *)0xeeeeeeee; } } while (0);
}
...
{
        struct mem_pool *mem_pool = ((void *)0);
        unsigned long padded_sizeof_type = 0;
        __attribute__((unused)) void *pool = ((void *)0);
        __attribute__((unused)) int i = 0;
        int ret = 0;
        __attribute__((unused)) struct list_head *list = ((void *)0);
        glusterfs_ctx_t *ctx = ((void *)0);
...
# 354 "./mem-pool.c"
        ...
        padded_sizeof_type = sizeof_type + ((sizeof(struct list_head)) + (sizeof(struct mem_pool*)) + sizeof(int));

        mem_pool = __gf_calloc (sizeof (*mem_pool), 1, gf_common_mt_mem_pool, "gf_common_mt_mem_pool");
        if (!mem_pool)
                return ((void *)0);
        ...
        pthread_mutex_init (&mem_pool->lock, 0);
        do { (&mem_pool->list)->next = (&mem_pool->list)->prev = &mem_pool->list; } while (0);
        do { (&mem_pool->global_list)->next = (&mem_pool->global_list)->prev = &mem_pool->global_list; } while (0);
        ...
        pool = __gf_calloc (count, padded_sizeof_type, gf_common_mt_long, "gf_common_mt_long");
        if (!pool) {
                __gf_free (mem_pool->name);
                __gf_free (mem_pool);
                return ((void *)0);
        }
...
{
        struct list_head *list = ((void *)0);
        void *ptr = ((void *)0);
        int *in_use = ((void *)0);
        struct mem_pool **pool_ptr = ((void *)0);
...
# 438 "./mem-pool.c"
        ...
        pthread_mutex_lock (&mem_pool->lock);
        {
                ...
                if (mem_pool->cold_count) {
                        ...
                        ptr = list;
                        in_use = (ptr + (sizeof(struct list_head)) +
...
# 484 "./mem-pool.c"
                ...
                ptr = __gf_calloc (1, mem_pool->padded_sizeof_type, gf_common_mt_mem_pool, "gf_common_mt_mem_pool")
                                                       ;
...
fwd_addr_out:
        pool_ptr = ((ptr) + (sizeof(struct list_head)));
        *pool_ptr = (struct mem_pool *)mem_pool;
        ptr = ((ptr) + ((sizeof(struct list_head)) + (sizeof(struct mem_pool*)) + sizeof(int)));
...
# 510 "./mem-pool.c"
        ...
        if ((((ptr) - ((sizeof(struct list_head)) + (sizeof(struct mem_pool*)) + sizeof(int))) - pool->pool)
...
mem_put (void *ptr)
{
        ...
        int *in_use = ((void *)0);
        void *head = ((void *)0);
        struct mem_pool **tmp = ((void *)0);
        struct mem_pool *pool = ((void *)0);
...
# 536 "./mem-pool.c"
        ...
        list = head = ((ptr) - ((sizeof(struct list_head)) + (sizeof(struct mem_pool*)) + sizeof(int)));
        tmp = ((head) + (sizeof(struct list_head)));
        if (!tmp) {
                do { _gf_msg ("mem-pool",
...
# 544 "./mem-pool.c"
 ...
 546
# 544 "./mem-pool.c"
                , GF_LOG_ERROR, 0, 1, (100000 + 1000 + 155), "ptr header is corrupted"); } while (0)
...
# 552 "./mem-pool.c"
 ...
 554
# 552 "./mem-pool.c"
                , GF_LOG_ERROR, 0, 1, (100000 + 1000 + 167), "mem-pool ptr is NULL"); } while (0)
        ...
        pthread_mutex_lock (&pool->lock);
        {
                ...
                {
                case 1:
                        in_use = (head + (sizeof(struct list_head)) +
                                  (sizeof(struct mem_pool*)));
                        if (!(*in_use == 1)) {
                                do { _gf_msg ("mem-pool",
...
# 566 "./mem-pool.c"
 ...
 571
# 566 "./mem-pool.c"
                                , GF_LOG_CRITICAL, 0, 1, (100000 + 1000 + 200), "mem_put called on freed ptr" " %p of mem pool %p", ptr, pool); } while (0)
...
# 614 "./mem-pool.c"
 ...
 616
# 614 "./mem-pool.c"
        , GF_LOG_INFO, 0, 0, (100000 + 1000 + 53), "size=%lu " "max=%d total=%""l" "u", pool->padded_sizeof_type, pool->max_alloc, pool->alloc_count); } while (0)
