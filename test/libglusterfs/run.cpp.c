...
runinit (runner_t *runner)
{
        ...
        runner->argvlen = 64;
        runner->argv = __gf_calloc (runner->argvlen, sizeof (*runner->argv), gf_common_mt_run_argv, "gf_common_mt_run_argv")

                                                        ;
        runner->runerr = runner->argv ? 0 : (*__errno_location ());
        runner->chpid = -1;
        for (i = 0; i < 3; i++) {
                runner->chfd[i] = -1;
                runner->chio[i] = ((void *)0);
        }
}
...
{
        do { if (!(fd > 0 && fd < 3)) { do { _gf_msg ("", "./run.c", __FUNCTION__, 69, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "fd > 0 && fd < 3"); } while (0); } } while (0);
...
runner_insert_arg (runner_t *runner, char *arg)
{
        ...
        do { if (!(arg)) { do { _gf_msg ("", "./run.c", __FUNCTION__, 82, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "arg"); } while (0); } } while (0);

        if (runner->runerr)
                return;

        for (i = 0; i < runner->argvlen; i++) {
                if (runner->argv[i] == ((void *)0))
                        break;
        }
        do { if (!(i < runner->argvlen)) { do { _gf_msg ("", "./run.c", __FUNCTION__, 91, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "i < runner->argvlen"); } while (0); } } while (0);

        if (i == runner->argvlen - 1) {
                runner->argv = __gf_realloc (runner->argv, runner->argvlen * 2 * sizeof (*runner->argv))
                                                                                        ;
                if (!runner->argv) {
                        runner->runerr = (*__errno_location ());
                        return;
                }
                memset (
...
runner_va_add_args (runner_t *runner, va_list argp)
{
        ...
        while ((arg = __builtin_va_arg(argp,const char *)))
...
runner_add_args (runner_t *runner, ...)
{
        ...
        __builtin_va_start(argp,runner);
            ...
            const char *msg)
{
        ...
        buf = __gf_calloc (1, len + 1, gf_common_mt_run_logbuf, "gf_common_mt_run_logbuf");
        if (!buf) {
                runner->runerr = (*__errno_location ());
                return;
        }
        ...
        do { _gf_msg (dom, "./run.c", __FUNCTION__, 191, lvl, 0, 1, (100000 + 1000 + 204), "%s: %s", msg, buf); } while (0);
...
runner_redir (runner_t *runner, int fd, int tgt_fd)
{
        do { if (!(fd > 0 && fd < 3)) { do { _gf_msg ("", "./run.c", __FUNCTION__, 199, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "fd > 0 && fd < 3"); } while (0); } } while (0);
...
runner_start (runner_t *runner)
{
        ...
        if (runner->runerr) {
                (*__errno_location ()) = runner->runerr;
                return -1;
        }

        do { if (!(runner->argv[0])) { do { _gf_msg ("", "./run.c", __FUNCTION__, 220, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "runner->argv[0]"); } while (0); } } while (0);
...
# 294 "./run.c"
                        ...
                        do { if (!(ret == 0)) { do { _gf_msg ("", "./run.c", __FUNCTION__, 296, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "ret == 0"); } while (0); } } while (0);
                ...
                ret = sys_write (xpi[1], &(*__errno_location ()), sizeof ((*__errno_location ())));
                _exit (1);
        }
        ...
        if (ret == -1) {
                ...
                if (ret <= 0)
                        return 0;
                do { if (!(ret == sizeof (errno_priv))) { do { _gf_msg ("", "./run.c", __FUNCTION__, 332, GF_LOG_ERROR, 0, 1, (100000 + 1000 + 150), "Assertion failed: " "ret == sizeof (errno_priv)"); } while (0); } } while (0);
        }
        (*__errno_location ()) = errno_priv;
        return -1;
}
