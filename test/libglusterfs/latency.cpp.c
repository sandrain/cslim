...
gf_latency_begin (call_frame_t *frame, void *fn)
{
        ...
        gettimeofday (&frame->begin, ((void *)0));
}
...
{
        char key_prefix[4096];
        char key[4096];
        int i;

        snprintf (key_prefix, 4096, "%s.latency", xl->name);
        gf_proc_dump_add_section (key_prefix);

        for (i = 0; i < GF_FOP_MAXVALUE; i++) {
                { _gf_proc_dump_build_key(key, key_prefix, (char *)gf_fop_list[i]); }
                                                               ;

                gf_proc_dump_write (key, "%.03f,%""l" "d"",%.03f",
                                    xl->latencies[i].mean,
...
# 185 "./latency.c"
 ...
 188
# 185 "./latency.c"
                , GF_LOG_INFO, 0, 0, (100000 + 1000 + 43), "Latency measurement turned %s", ctx->measure_latency ? "on" : "off"); } while (0)
