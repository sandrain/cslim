...
gf_timer_call_after (glusterfs_ctx_t *ctx,
                     ...
                     void *data)
{
        gf_timer_registry_t *reg = ((void *)0);
        gf_timer_t *event = ((void *)0);
        gf_timer_t *trav = ((void *)0);
        uint64_t at = 0;
...
# 41 "./timer.c"
 ...
 43
# 41 "./timer.c"
                , GF_LOG_INFO, 0, 1, (100000 + 1000 + 153), "ctx cleanup " "started"); } while (0)
...
# 50 "./timer.c"
 ...
 51
# 50 "./timer.c"
                , GF_LOG_ERROR, 0, 1, (100000 + 1000 + 154), "!reg"); } while (0)
                                                                      ;
                return ((void *)0);
        }

        event = __gf_calloc (1, sizeof (*event), gf_common_mt_gf_timer_t, "gf_common_mt_gf_timer_t");
        if (!event) {
                return ((void *)0);
        }
        timespec_now (&event->at);
        timespec_adjust_delta (&event->at, delta);
        at = ((event->at.tv_sec * 1000000000LL) + event->at.tv_nsec);
        event->callbk = callbk;
        event->data = data;
        event->xl = (*__glusterfs_this_location());
        pthread_mutex_lock (&reg->lock);
        {
                trav = reg->active.prev;
                while (trav != &reg->active) {
                        if (((trav->at.tv_sec * 1000000000LL) + trav->at.tv_nsec) < at)
                                break;
                        trav = trav->prev;
                }
                event->prev = trav;
...
# 98 "./timer.c"
 ...
 99
# 98 "./timer.c"
                , GF_LOG_ERROR, 0, 0, (100000 + 1000 + 47), "!reg"); } while (0)
                               ;
                __gf_free (event);
                return 0;
        }
...
# 146 "./timer.c"
        ...
        while (!reg->fin) {
                ...
                timespec_now (&now_ts);
                now = ((now_ts.tv_sec * 1000000000LL) + now_ts.tv_nsec);
                while (1) {
                        uint64_t at;
                        char need_cbk = 0;
                        ...
                        if (need_cbk) {
                                ...
                                if (event->xl) {
                                        ...
                                        (*__glusterfs_this_location()) = event->xl;
                                }
        ...
        __gf_free (((glusterfs_ctx_t *)ctx)->timer);
...
# 214 "./timer.c"
        ...
        if (!ctx->timer) {
                ...
                reg = __gf_calloc (1, sizeof (*reg), gf_common_mt_gf_timer_registry_t, "gf_common_mt_gf_timer_registry_t")
                                                                  ;
                if (!reg)
                        goto out;

                pthread_mutex_init (&reg->lock, ((void *)0));
                reg->active.next = &reg->active;
                reg->active.prev = &reg->active;

                ctx->timer = reg;
                gf_thread_create (&reg->th, ((void *)0), gf_timer_proc, ctx);
        }
out:
        return ctx->timer;
}
