        ...
        void (*destroy_buffer_data) (void *data))
{
        ...
        history = __gf_calloc (1, sizeof (eh_t), gf_common_mt_eh_t, "gf_common_mt_eh_t");
        if (!history) {
                goto out;
        }
        ...
        pthread_mutex_init (&history->lock, ((void *)0));
out:
        return history;
}
...
{
        if (!history) {
                do { _gf_msg ("event-history", "./event-history.c", __FUNCTION__, 46, GF_LOG_DEBUG, 0, 0, 0, "history is NULL"); } while (0);
                goto out;
        }
...
{
        if (!history) {
                do { _gf_msg ("event-history",
 "./event-history.c"
# 70 "./event-history.c"
                , __FUNCTION__,
 71
# 70 "./event-history.c"
                , GF_LOG_INFO, 0, 0, (100000 + 1000 + 172), "history for the xlator is NULL"); } while (0)
                                                         ;
                return -1;
        }

        cb_buffer_destroy (history->buffer);
        history->buffer = ((void *)0);
