...
glusterfs_ctx_new ()
{
 ...
 glusterfs_ctx_t *ctx = ((void *)0);
        ...
        do { (&ctx->graphs)->next = (&ctx->graphs)->prev = &ctx->graphs; } while (0);
 do { (&ctx->mempool_list)->next = (&ctx->mempool_list)->prev = &ctx->mempool_list; } while (0);

 ctx->daemon_pipe[0] = -1;
 ctx->daemon_pipe[1] = -1;

 ret = pthread_mutex_init (&ctx->lock, ((void *)0));
 if (ret) {
  free (ctx);
  ctx = ((void *)0);
 }
